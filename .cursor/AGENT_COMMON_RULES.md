...
## ⚠️ КРИТИЧЕСКИ ВАЖНО: Краткость и фокус на коде
...
**ВСЕГДА ДЕЛАЙ:**
- ✅ Работай с кодом напрямую
- ✅ Краткие ответы (1-2 предложения)
- ✅ Показывай только код и изменения
- ✅ Минимум текста, максимум действий
- ✅ Фокус на реализации

**Комментарии:**
- При передаче: `✅ Ready. Handed off to {NextAgent}. Issue: #{number}`
- При возврате: `⚠️ Returned: {reason}. Issue: #{number}`
- НЕ пиши длинные списки изменений

---

## ⚠️ КРИТИЧЕСКИ ВАЖНО: Git команды

### ✅ РАЗРЕШЕННЫЕ git операции:
...
Если сделал ошибку:
- ✅ Создай новый коммит с исправлением
- ✅ Используй `git revert <commit>` для отмены коммита
...
**⚠️ КРИТИЧНО: Backend ОБЯЗАН применять оптимизации для MMOFPS RPG**
...
- ✅ Только краткое подтверждение готовности
- ✅ Только номер Issue
...
- ✅ Только причина и правильный агент
