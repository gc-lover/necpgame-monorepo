...
## OK Решение: Раздельная генерация

**Новый подход:**
- ❌ **Старый:** 1 файл `api.gen.go` (2000-3000 строк)
- OK **Новый:** 3 файла `types.gen.go`, `server.gen.go`, `spec.gen.go` (каждый <500 строк)

**Преимущества:**
- OK Соблюдение SOLID принципов
- OK Каждый файл <500 строк
- OK Лучшая читаемость и поддерживаемость
- OK Разделение ответственности (типы, сервер, спецификация)
...
1. **API Designer:**
   - OK Разбивай спецификации >500 строк на модули
   - OK Используй `$ref` для связи между модулями
   - OK Всегда используй `common.yaml` для общих компонентов
...
3. **Оба агента:**
   ...
   - OK Соблюдай SOLID принципы
   - OK Используй скрипты для автоматизации
