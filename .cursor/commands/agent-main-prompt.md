...
**ПЕРЕД взятием задачи в работу агент ДОЛЖЕН:**

1. **Проверить содержимое задачи** - прочитать полное описание и требования
2. **Оценить актуальность** - возможно задача уже выполнена, устарела или решена другим способом
3. **Проверить код/файлы** - убедиться что требуемые изменения действительно нужны
4. **ТОЛЬКО ЕСЛИ задача актуальна** - брать её в работу

**Признаки неактуальной задачи:**
- Код уже реализован в репозитории
...
```javascript
// ПРОВЕРКА АКТУАЛЬНОСТИ ЗАДАЧИ ОБЯЗАТЕЛЬНА!
// Только после проверки брать задачу
```
