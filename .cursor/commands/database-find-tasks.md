...
1. **Search in Project by Status:**
   ...
   mcp_github_list_project_items({
     ...
     owner: 'gc-lover',
     project_number: 1,
     query: 'Status:"Database - Todo" OR Status:"Database - In Progress"'
   });
   ```
...
4. **При выборе задачи:** ОБЯЗАТЕЛЬНО обнови статус на `Database - In Progress`:
   ...
   **Примечание:** `item_id` используется только для API вызова. В комментариях и сообщениях всегда указывай номер Issue (например, `Issue: #123`).
   ...
   mcp_github_update_project_item({
     ...
     updated_field: {
       id: 239690516,  // число
       value: '{option_id}'  // id опции '91d49623' из list_project_fields  // id опции "Database - In Progress"
     }
   });
   ```
