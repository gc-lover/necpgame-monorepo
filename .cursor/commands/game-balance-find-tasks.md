...
1. **Search in Project by Status:**
   ...
   mcp_github_list_project_items({
     ...
     owner: 'gc-lover',
     project_number: 1,
     query: 'Status:"Game Balance - Todo" OR Status:"Game Balance - In Progress"'
   });
   ```
...
3. **При выборе задачи:** ОБЯЗАТЕЛЬНО обнови статус на `Game Balance - In Progress`:
   ...
   **Примечание:** `item_id` используется только для API вызова. В комментариях и сообщениях всегда указывай номер Issue (например, `Issue: #123`).
   ...
   mcp_github_update_project_item({
     ...
     updated_field: {
       id: 239690516,  // число
       value: '{option_id}'  // id опции '{option_id}' из list_project_fields  // id опции "Game Balance - In Progress"
     }
   });
   ```
