...
## ⚠️ Автоматическая валидация меток

GitHub Action автоматически проверяет метки при создании и редактировании Issues. При нарушении правил workflow:
- Создается комментарий с описанием ошибок
- Автоматически исправляются несовместимые метки
- Удаляются лишние метки агентов

**См. документацию:** `.github/LABEL_VALIDATION.md`

## Общие правила

Каждый агент ОБЯЗАН управлять метками при работе с Issue:

1. **При начале работы** - добавить свою метку `agent:{agent-name}` и соответствующую `stage:{stage-name}`
2. **При завершении работы** - удалить свою метку
3. **При переходе к следующему этапу** - добавить метку следующего агента и соответствующую метку этапа
