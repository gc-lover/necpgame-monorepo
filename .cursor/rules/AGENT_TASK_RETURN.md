...
```markdown
WARNING **Задача возвращена: отсутствуют входные данные**
...
```markdown
WARNING **Задача возвращена: неправильный тип задачи**
...
```markdown
WARNING **Задача возвращена: требуется ручное вмешательство**
...
async function returnTask(issueNumber, reason, correctAgent) {
  ...
  await mcp_github_add_issue_comment({
    ...
    repo: 'necpgame-monorepo',
    issue_number: issueNumber,
    body: `WARNING **Задача возвращена: ${reason}**\n\n[Детали возврата]`
  });
