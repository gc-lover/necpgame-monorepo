...
## WARNING ОСНОВНЫЕ МЕТОДЫ ОПТИМИЗАЦИИ
...
**WARNING ОГРАНИЧЕНИЕ:** Кэш работает только в рамках одного вызова агента (одна сессия чата). Между разными вызовами агентов кэш не сохраняется.
...
**WARNING КРИТИЧЕСКИ ВАЖНО:** Кэширование в памяти сессии ОБЯЗАТЕЛЬНО для всех агентов.
...
**WARNING КРИТИЧЕСКИ ВАЖНО:** Для получения списка Issues ВСЕГДА используй поиск, а не множественные `issue_read`.
...
- OK Нужно обработать **>10 Issues** одновременно
- OK Операция может быть выполнена асинхронно
- OK Нужна гарантия защиты от rate limit
- OK Операция повторяющаяся (можно автоматизировать)

### Используй прямые MCP запросы если:
- OK Нужно обработать **<10 Issues**
- OK Требуется немедленная обратная связь
- OK Операция уникальная и не повторяется
- OK Нужна интерактивная обработка
