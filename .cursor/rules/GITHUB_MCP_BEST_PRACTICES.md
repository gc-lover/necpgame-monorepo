...
## WARNING ВАЖНО: Основные методы оптимизации
...
## OK Правильные практики

### 1. Использование поиска (ГЛАВНОЕ!)

```javascript
// OK ПРАВИЛЬНО: Используем поиск вместо множественных запросов
...
```javascript
// OK ПРАВИЛЬНО: Кэширование в рамках одной сессии
...
```javascript
// OK ПРАВИЛЬНО: Последовательные запросы с задержками
...
```javascript
// OK ПРАВИЛЬНО: Обработка rate limit с повторами
...
```javascript
// OK ПРАВИЛЬНО: Для >10 Issues используем GitHub Actions
