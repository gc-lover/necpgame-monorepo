...
- ✅ Убраны `set -e` для предотвращения преждевременного завершения
- ✅ Улучшена обработка ошибок с явными проверками
- ✅ Используются временные файлы вместо process substitution
- ✅ Поддержка Windows-переменных окружения (TMPDIR, TMP)
- ✅ Автоматический обход при ошибках (hooks не блокируют git команды)
