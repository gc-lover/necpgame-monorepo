...
static FName
    NAME_LyraCharacterCollisionProfile_Capsule(TEXT("LyraPawnCapsule"));
static FName NAME_LyraCharacterCollisionProfile_Mesh(TEXT("LyraPawnMesh"));

ALyraCharacter::ALyraCharacter(const FObjectInitializer &ObjectInitializer)
    : Super(ObjectInitializer
                .SetDefaultSubobjectClass<ULyraCharacterMovementComponent>(
  ...
  check(MeshComp);
  MeshComp->SetRelativeRotation(FRotator(
      0.0f, -90.0f,
      0.0f)); // Rotate mesh to be X forward since it is exported as Y forward.
  ...
  PawnExtComponent = CreateDefaultSubobject<ULyraPawnExtensionComponent>(
      TEXT("PawnExtensionComponent"));
  ...
  HealthComponent =
      CreateDefaultSubobject<ULyraHealthComponent>(TEXT("HealthComponent"));
  HealthComponent->OnDeathStarted.AddDynamic(this, &ThisClass::OnDeathStarted);
  HealthComponent->OnDeathFinished.AddDynamic(this,
                                              &ThisClass::OnDeathFinished);
...
void ALyraCharacter::BeginPlay() {
  ...
  bool bWasReplicatingMovement = false;
  UWorld *World = GetWorld();
  if (World) {
    ENetMode NetMode = World->GetNetMode();
    if (NetMode != NM_Standalone) {
      bWasReplicatingMovement = true;
    }
  }

  UpdateMovementReplication();

  if (World) {
    bool bShouldReplicateMovement = false;
    if (GetLocalRole() == ROLE_Authority) {
      ALyraGameMode *GameMode = World->GetAuthGameMode<ALyraGameMode>();
      if (GameMode && GameMode->GetGatewayConnection()) {
        bShouldReplicateMovement =
            GameMode->GetGatewayConnection()->IsConnected();
      }
    } else if (GetLocalRole() == ROLE_SimulatedProxy ||
               GetLocalRole() == ROLE_AutonomousProxy) {
      APlayerController *LocalPC = World->GetFirstPlayerController();
      if (LocalPC) {
        ALyraPlayerController *LyraPC = Cast<ALyraPlayerController>(LocalPC);
        if (LyraPC && LyraPC->GetWebSocketConnection()) {
          bShouldReplicateMovement =
              LyraPC->GetWebSocketConnection()->IsConnected();
        }
      }
    }
  ...
  if (bRegisterWithSignificanceManager) {
    if (ULyraSignificanceManager *SignificanceManager =
            USignificanceManager::Get<ULyraSignificanceManager>(World)) {
      //@TODO: SignificanceManager->RegisterObject(this,
      //(EFortSignificanceType)SignificanceType);
    }
  }
}
...
void ALyraCharacter::GetLifetimeReplicatedProps(
    TArray<FLifetimeProperty> &OutLifetimeProps) const {
  Super::GetLifetimeReplicatedProps(OutLifetimeProps);

  DOREPLIFETIME_CONDITION(ThisClass, ReplicatedAcceleration,
                          COND_SimulatedOnly);
  DOREPLIFETIME(ThisClass, MyTeamID)
}

void ALyraCharacter::PreReplication(
    IRepChangedPropertyTracker &ChangedPropertyTracker) {
  Super::PreReplication(ChangedPropertyTracker);

  if (UCharacterMovementComponent *MovementComponent = GetCharacterMovement()) {
    // Compress Acceleration: XY components as direction + magnitude, Z
    // component as direct value
    const double MaxAccel = MovementComponent->MaxAcceleration;
    const FVector CurrentAccel = MovementComponent->GetCurrentAcceleration();
    double AccelXYRadians, AccelXYMagnitude;
    FMath::CartesianToPolar(CurrentAccel.X, CurrentAccel.Y, AccelXYMagnitude,
                            AccelXYRadians);

    ReplicatedAcceleration.AccelXYRadians = FMath::FloorToInt(
        (AccelXYRadians / TWO_PI) * 255.0); // [0, 2PI] -> [0, 255]
    ReplicatedAcceleration.AccelXYMagnitude = FMath::FloorToInt(
        (AccelXYMagnitude / MaxAccel) * 255.0); // [0, MaxAccel] -> [0, 255]
    ReplicatedAcceleration.AccelZ =
        FMath::FloorToInt((CurrentAccel.Z / MaxAccel) *
                          127.0); // [-MaxAccel, MaxAccel] -> [-127, 127]
  }
}
...
void ALyraCharacter::NotifyControllerChanged() {
  ...
  if (HasAuthority() && (GetController() != nullptr)) {
    if (ILyraTeamAgentInterface *ControllerWithTeam =
            Cast<ILyraTeamAgentInterface>(GetController())) {
...
ALyraPlayerController *ALyraCharacter::GetLyraPlayerController() const {
  return CastChecked<ALyraPlayerController>(GetController(),
                                            ECastCheckedType::NullAllowed);
}

ALyraPlayerState *ALyraCharacter::GetLyraPlayerState() const {
  return CastChecked<ALyraPlayerState>(GetPlayerState(),
                                       ECastCheckedType::NullAllowed);
}

ULyraAbilitySystemComponent *
ALyraCharacter::GetLyraAbilitySystemComponent() const {
...
void ALyraCharacter::PossessedBy(AController *NewController) {
  ...
  if (ILyraTeamAgentInterface *ControllerAsTeamProvider =
          Cast<ILyraTeamAgentInterface>(NewController)) {
...
void ALyraCharacter::UnPossessed() {
  ...
  const FGenericTeamId OldTeamID = MyTeamID;
  if (ILyraTeamAgentInterface *ControllerAsTeamProvider =
          Cast<ILyraTeamAgentInterface>(OldController)) {
...
void ALyraCharacter::SetupPlayerInputComponent(
...
void ALyraCharacter::InitializeGameplayTags() {
  // Clear tags that may be lingering on the ability system from the previous
  // pawn.
  if (ULyraAbilitySystemComponent *LyraASC = GetLyraAbilitySystemComponent()) {
    for (const TPair<uint8, FGameplayTag> &TagMapping :
         LyraGameplayTags::MovementModeTagMap) {
    ...
    ULyraCharacterMovementComponent *LyraMoveComp =
        CastChecked<ULyraCharacterMovementComponent>(GetCharacterMovement());
    SetMovementModeTag(LyraMoveComp->MovementMode,
                       LyraMoveComp->CustomMovementMode, true);
  }
}

void ALyraCharacter::GetOwnedGameplayTags(
...
bool ALyraCharacter::HasAllMatchingGameplayTags(
...
bool ALyraCharacter::HasAnyMatchingGameplayTags(
    const FGameplayTagContainer &TagContainer) const {
  if (const ULyraAbilitySystemComponent *LyraASC =
          GetLyraAbilitySystemComponent()) {
...
void ALyraCharacter::OnDeathStarted(AActor *) { DisableMovementAndCollision(); }
...
void ALyraCharacter::UninitAndDestroy() {
  ...
  // Uninitialize the ASC if we're still the avatar actor (otherwise another
  // pawn already did it when they became the avatar actor)
  if (ULyraAbilitySystemComponent *LyraASC = GetLyraAbilitySystemComponent()) {
...
void ALyraCharacter::OnMovementModeChanged(EMovementMode PrevMovementMode,
...
void ALyraCharacter::SetMovementModeTag(EMovementMode MovementMode,
  ...
  if (ULyraAbilitySystemComponent *LyraASC = GetLyraAbilitySystemComponent()) {
    ...
    if (MovementMode == MOVE_Custom) {
      MovementModeTag =
          LyraGameplayTags::CustomMovementModeTagMap.Find(CustomMovementMode);
    } else {
      MovementModeTag = LyraGameplayTags::MovementModeTagMap.Find(MovementMode);
    }

    if (MovementModeTag && MovementModeTag->IsValid()) {
      LyraASC->SetLooseGameplayTagCount(*MovementModeTag,
                                        (bTagEnabled ? 1 : 0));
    }
  }
}
...
void ALyraCharacter::OnStartCrouch(float HalfHeightAdjust,
...
void ALyraCharacter::OnEndCrouch(float HalfHeightAdjust,
                                 float ScaledHalfHeightAdjust) {
  if (ULyraAbilitySystemComponent *LyraASC = GetLyraAbilitySystemComponent()) {
...
void ALyraCharacter::OnRep_ReplicatedAcceleration() {
  if (ULyraCharacterMovementComponent *LyraMovementComponent =
    ...
    const double AccelXYMagnitude =
        double(ReplicatedAcceleration.AccelXYMagnitude) * MaxAccel /
        255.0; // [0, 255] -> [0, MaxAccel]
    const double AccelXYRadians =
        double(ReplicatedAcceleration.AccelXYRadians) * TWO_PI /
        255.0; // [0, 255] -> [0, 2PI]

    FVector UnpackedAcceleration(FVector::ZeroVector);
    FMath::PolarToCartesian(AccelXYMagnitude, AccelXYRadians,
                            UnpackedAcceleration.X, UnpackedAcceleration.Y);
    UnpackedAcceleration.Z = double(ReplicatedAcceleration.AccelZ) * MaxAccel /
                             127.0; // [-127, 127] -> [-MaxAccel, MaxAccel]
  ...
  if (GetController() == nullptr) {
    if (HasAuthority()) {
      ...
      UE_LOG(LogLyraTeams, Error,
             TEXT("You can't set the team ID on a character (%s) except on the "
                  "authority"),
             *GetPathNameSafe(this));
    }
  } else {
    UE_LOG(LogLyraTeams, Error,
           TEXT("You can't set the team ID on a possessed character (%s); it's "
                "driven by the associated controller"),
           *GetPathNameSafe(this));
  }
}

FGenericTeamId ALyraCharacter::GetGenericTeamId() const { return MyTeamID; }

FOnLyraTeamIndexChangedDelegate *
ALyraCharacter::GetOnTeamIndexChangedDelegate() {
  return &OnTeamChangedDelegate;
}

void ALyraCharacter::OnControllerChangedTeam(UObject *TeamAgent, int32 OldTeam,
                                             int32 NewTeam) {
...
void ALyraCharacter::UpdateMovementReplication() {
  UWorld *World = GetWorld();
  if (!World) {
    return;
  }
  ...
  SetReplicateMovement(bShouldReplicateMovement);
}
...
bool ALyraCharacter::UpdateSharedReplication() {
  if (GetLocalRole() == ROLE_Authority) {
    ...
    if (!GameMode->GetGatewayConnection()->IsConnected()) {
      return false;
    }
...
void ALyraCharacter::FastSharedReplication_Implementation(
    const FSharedRepMovement &SharedRepMovement) {
  ...
  if (GetLocalRole() == ROLE_SimulatedProxy) {
    ...
    if (World) {
      if (APlayerController *LocalPC = World->GetFirstPlayerController()) {
        if (ALyraPlayerController *LyraPC =
                Cast<ALyraPlayerController>(LocalPC)) {
          if (URealtimeWebSocketConnection *WebSocketConn =
                  LyraPC->GetWebSocketConnection()) {
            if (WebSocketConn->IsConnected()) {
              UE_LOG(LogLyra, Warning,
                     TEXT("ALyraCharacter::FastSharedReplication_"
                          "Implementation: ===== WebSocket connected, SKIPPING "
                          "standard replication for %s ====="),
                     *GetName());
              return;
            }
          }
    ...
    SetReplicatedServerLastTransformUpdateTimeStamp(
        SharedRepMovement.RepTimeStamp);
...
bool FSharedRepMovement::FillForCharacter(ACharacter *Character) {
  if (USceneComponent *PawnRootComponent = Character->GetRootComponent()) {
    ...
    if ((CharacterMovement->NetworkSmoothingMode ==
         ENetworkSmoothingMode::Linear) ||
...
bool FSharedRepMovement::Equals(const FSharedRepMovement &Other,
...
bool FSharedRepMovement::NetSerialize(FArchive &Ar, class UPackageMap *Map,
                                      bool &bOutSuccess) {
  bOutSuccess = true;
