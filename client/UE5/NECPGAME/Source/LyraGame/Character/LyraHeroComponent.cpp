	...
	if (const APawn* Pawn = GetPawn<APawn>())
	{
		if (const ULyraPawnExtensionComponent* PawnExtComp = ULyraPawnExtensionComponent::FindPawnExtensionComponent(Pawn))
		{
			if (ULyraAbilitySystemComponent* LyraASC = PawnExtComp->GetLyraAbilitySystemComponent())
			{
				...
				if (InputTagString.Contains(TEXT("Fire")) || InputTagString.Contains(TEXT("Shoot")) || InputTagString.Contains(TEXT("Weapon")))
				{
					bIsShooting = true;
...
void ULyraHeroComponent::SendPlayerInputToWebSocket(float MoveX, float MoveY, bool bShoot, float AimX, float AimY)
{
	APawn* Pawn = GetPawn<APawn>();
	if (!Pawn)
	{
	...
	ALyraPlayerController* LyraController = Cast<ALyraPlayerController>(Controller);
	if (!LyraController)
	{
		return;
	}

	URealtimeWebSocketConnection* WebSocketConn = LyraController->GetWebSocketConnection();
	if (!WebSocketConn)
	{
		UE_LOG(LogLyra, VeryVerbose, TEXT("ULyraHeroComponent::SendPlayerInputToWebSocket: WebSocketConnection is NULL"));
		return;
	}
	
	if (!WebSocketConn->IsConnected())
	{
		UE_LOG(LogLyra, VeryVerbose, TEXT("ULyraHeroComponent::SendPlayerInputToWebSocket: WebSocketConnection not connected"));
		return;
	}

	UE_LOG(LogLyra, Verbose, TEXT("ULyraHeroComponent::SendPlayerInputToWebSocket: Sending input MoveX=%.2f, MoveY=%.2f, Shoot=%d, AimX=%.2f, AimY=%.2f"), 
		MoveX, MoveY, bShoot ? 1 : 0, AimX, AimY);
	WebSocketConn->SendPlayerInput(MoveX, MoveY, bShoot, AimX, AimY);
}
