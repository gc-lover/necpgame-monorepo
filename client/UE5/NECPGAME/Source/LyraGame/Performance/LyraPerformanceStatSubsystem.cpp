...
void FLyraPerformanceStatCache::ProcessFrame(const FFrameData& FrameData)
{
	...
	if (UWorld* World = MySubsystem->GetGameInstance()->GetWorld())
	{
		...
		if (APlayerController* LocalPC = GEngine->GetFirstLocalPlayerController(World))
		{
			...
			{
				double PingValue = PS->GetPingInMilliseconds();
				
				if (ALyraPlayerController* LyraPC = Cast<ALyraPlayerController>(LocalPC))
				{
					if (URealtimeWebSocketConnection* WebSocketConn = LyraPC->GetWebSocketConnection())
					{
						if (WebSocketConn->IsConnected())
						{
							int32 WebSocketRTT = WebSocketConn->GetLastRTT();
							if (WebSocketRTT > 0)
							{
								PingValue = static_cast<double>(WebSocketRTT);
							}
						}
					}
				}
				
				RecordStat(ELyraDisplayablePerformanceStat::Ping, PingValue);
			}
