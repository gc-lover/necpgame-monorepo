...
public:
	...
	UE_API URealtimeWebSocketConnection* GetWebSocketConnection() const { return WebSocketConnection; }

	UFUNCTION(BlueprintCallable, Category = "WebSocket")
	UE_API void ConnectToWebSocketServer(const FString& ServerAddress, int32 ServerPort, const FString& Token);

	UFUNCTION(BlueprintCallable, Category = "WebSocket")
	UE_API void DisconnectFromWebSocketServer();

	UFUNCTION()
	UE_API void OnGameStateReceived(const TArray<uint8>& GameStateData);

	static FString GenerateShortPlayerId(const FString& OriginalId);

	UFUNCTION()
	UE_API void OnWebSocketConnected(bool bSuccess);

	UFUNCTION()
	UE_API void OnWebSocketDisconnected(const FString& Reason);
