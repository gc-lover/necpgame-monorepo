		...
		[this](UClass* Class, FClassReplicationInfo& ClassInfo)
		{
			...
			if (Lyra::RepGraph::LogLazyInitClasses != 0)
			{
				if (bHandled)
				{
					...
					// UE 5.6+ API change: GetClassInfo removed, using iterator instead
					UClass* SuperClass = Class->GetSuperClass();
					if (SuperClass)
					{
						for (auto ClassRepInfoIt = GlobalActorReplicationInfoMap.CreateClassMapIterator(); ClassRepInfoIt; ++ClassRepInfoIt)
						{
							UClass* ItClass = CastChecked<UClass>(ClassRepInfoIt.Key().ResolveObjectPtr());
							if (ItClass == SuperClass)
							{
								const FClassReplicationInfo& ParentRepInfo = ClassRepInfoIt.Value();
