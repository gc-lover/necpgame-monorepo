...
CREATE TRIGGER trigger_quest_definitions_updated_at
    BEFORE UPDATE ON gameplay.quest_definitions
    FOR EACH ROW
    EXECUTE FUNCTION update_quest_definitions_updated_at();

-- Comments for API documentation
COMMENT ON TABLE gameplay.quest_definitions IS 'Quest definitions for NECPGAME content management API';
COMMENT ON COLUMN gameplay.quest_definitions.id IS 'Unique quest identifier (UUID)';
COMMENT ON COLUMN gameplay.quest_definitions.title IS 'Quest title (max 200 chars)';
COMMENT ON COLUMN gameplay.quest_definitions.description IS 'Detailed quest description';
COMMENT ON COLUMN gameplay.quest_definitions.status IS 'Quest status: active, completed, archived';
COMMENT ON COLUMN gameplay.quest_definitions.level_min IS 'Minimum player level required (1-100)';
COMMENT ON COLUMN gameplay.quest_definitions.level_max IS 'Maximum player level allowed (1-100)';
COMMENT ON COLUMN gameplay.quest_definitions.rewards IS 'JSONB array of quest rewards (experience, currency, items, reputation)';
COMMENT ON COLUMN gameplay.quest_definitions.objectives IS 'JSONB array of quest objectives (kill, collect, deliver, explore, talk)';
COMMENT ON COLUMN gameplay.quest_definitions.metadata IS 'Additional metadata for quest customization';
COMMENT ON COLUMN gameplay.quest_definitions.created_at IS 'Quest creation timestamp';
COMMENT ON COLUMN gameplay.quest_definitions.updated_at IS 'Quest last update timestamp';
