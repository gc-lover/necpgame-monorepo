...
CREATE OR REPLACE FUNCTION update_npc_definitions_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_npc_definitions_updated_at
    BEFORE UPDATE ON narrative.npc_definitions
    FOR EACH ROW
...
COMMENT ON TABLE narrative.npc_definitions IS 'NPC definitions for NECPGAME content management API';
COMMENT ON COLUMN narrative.npc_definitions.id IS 'Unique NPC identifier (UUID)';
COMMENT ON COLUMN narrative.npc_definitions.name IS 'NPC name (max 100 chars)';
COMMENT ON COLUMN narrative.npc_definitions.faction IS 'NPC faction affiliation';
COMMENT ON COLUMN narrative.npc_definitions.location IS 'NPC primary location';
COMMENT ON COLUMN narrative.npc_definitions.role IS 'NPC role/function (merchant, guard, quest giver, etc.)';
COMMENT ON COLUMN narrative.npc_definitions.dialogue_id IS 'Reference to main dialogue tree';
COMMENT ON COLUMN narrative.npc_definitions.quest_ids IS 'Array of quest IDs this NPC participates in';
COMMENT ON COLUMN narrative.npc_definitions.appearance IS 'JSONB NPC appearance data (model, voice, animations)';
COMMENT ON COLUMN narrative.npc_definitions.stats IS 'JSONB NPC stats (health, level, attributes)';
COMMENT ON COLUMN narrative.npc_definitions.metadata IS 'Additional metadata for NPC customization';
COMMENT ON COLUMN narrative.npc_definitions.created_at IS 'NPC creation timestamp';
COMMENT ON COLUMN narrative.npc_definitions.updated_at IS 'NPC last update timestamp';
