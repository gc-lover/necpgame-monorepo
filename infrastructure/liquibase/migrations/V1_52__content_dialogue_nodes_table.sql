...
CREATE OR REPLACE FUNCTION update_dialogue_nodes_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_dialogue_nodes_updated_at
    BEFORE UPDATE ON narrative.dialogue_nodes
    FOR EACH ROW
    EXECUTE FUNCTION update_dialogue_nodes_updated_at();

-- Comments for API documentation
COMMENT ON TABLE narrative.dialogue_nodes IS 'Dialogue nodes for NECPGAME content management API';
COMMENT ON COLUMN narrative.dialogue_nodes.id IS 'Unique dialogue node identifier (UUID)';
COMMENT ON COLUMN narrative.dialogue_nodes.npc_id IS 'Reference to NPC who owns this dialogue';
COMMENT ON COLUMN narrative.dialogue_nodes.quest_id IS 'Reference to quest this dialogue belongs to (optional)';
COMMENT ON COLUMN narrative.dialogue_nodes.node_data IS 'JSONB dialogue node data (text, responses, next_node_id)';
COMMENT ON COLUMN narrative.dialogue_nodes.conditions IS 'JSONB conditions for showing this dialogue (quest_completed, item_owned, etc.)';
COMMENT ON COLUMN narrative.dialogue_nodes.actions IS 'JSONB actions to perform (start_quest, give_item, change_reputation)';
COMMENT ON COLUMN narrative.dialogue_nodes.metadata IS 'Additional metadata for dialogue customization';
COMMENT ON COLUMN narrative.dialogue_nodes.created_at IS 'Dialogue creation timestamp';
COMMENT ON COLUMN narrative.dialogue_nodes.updated_at IS 'Dialogue last update timestamp';
