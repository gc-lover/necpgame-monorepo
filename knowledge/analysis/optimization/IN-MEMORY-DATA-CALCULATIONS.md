...
**⚠️ ВАЖНО:** Это **MMO FPS RPG** игра - в первую очередь это **ШУТЕР** в онлайн мире! Все расчеты ориентированы на стрельбу как основной геймплей.
...
**⚠️ ВАЖНО:** Для FPS игры используется **ActiveCombatZoneState**, а не turn-based сессии!
...
- ✅ **Активные боевые зоны (FPS real-time!)** - НЕ turn-based сессии!
- ✅ **Состояние стрельбы (ShootingState)** - прицеливание, отдача, оружие
- ✅ **История выстрелов (ShotHistory)** - последние 4-64 выстрела для античита
- ✅ **Буфер попаданий (HitDetectionBuffer)** - валидация попаданий в реальном времени
- ✅ Позиции игроков в активных зонах
- ✅ Текущее состояние здоровья, способностей
- ✅ WebSocket/UDP соединения
- ✅ История последних N тиков (для delta compression)
- ✅ **Активные снаряды (Projectiles)** - для трассировки и валидации
...
**Что хранить в Redis:**
- ✅ Кэш данных персонажей (характеристики, статы)
- ✅ Активные сессии (TTL: 30 минут)
- ✅ Лоадауты игроков
- ✅ Кэш инвентаря (активные предметы)
- ✅ Leaderboards (real-time)
- ✅ Игровые события (для Event Bus)
...
- ✅ Персонажи (полные данные)
- ✅ Инвентарь (все предметы)
- ✅ История боевых сессий
- ✅ Прогресс квестов
- ✅ Экономика (транзакции, рынок)
- ✅ Социальные данные (друзья, гильдии)
...
- **Тикрейт:** 128 Hz ✅ (как VALORANT)
- **Latency:** <30ms ✅
- **Игроков:** 20 ✅
- **Протокол:** UDP ✅

#### GvG (Large-scale PvP)
- **Тикрейт:** 60-80 Hz ✅ (как Battlefield 2042)
- **Latency:** <50ms ✅
- **Игроков:** 200-400 ✅
- **Протокол:** UDP с spatial partitioning ✅

#### Massive War (MMO-scale)
- **Тикрейт:** 20-40 Hz ✅ (как PlanetSide 2)
- **Latency:** <100ms ✅
- **Игроков:** 2000+ ✅
- **Протокол:** UDP с кластером ✅
...
|-----------|--------|-------------|-----------------|
| PlayerState (базовый) | 112 байт | ✅ Оптимально | ⭐⭐⭐ Высокая |
| **ShootingState** | **265 байт** | ✅ Оптимально | ⭐⭐⭐ **КРИТИЧНО!** |
| **ShotHistory (32 выстрела)** | **7.1 KB** | Можно 16 для экономии | ⭐⭐⭐ **КРИТИЧНО!** |
| **WeaponState** | **288 байт** | ✅ Оптимально | ⭐⭐⭐ **КРИТИЧНО!** |
...
| **Итого (FPS оптимизированный)** | **~12.8 KB** | ✅ Рекомендуется | **FPS приоритет!** |
...
|----------|---------|--------|--------------|-----------------|
| PvE Зона | 500 | 2.3 MB | ✅ Достаточно 32 GB сервер | Базовая стрельба |
| **PvP Small** | **20** | **1.03 MB** | ✅ Достаточно 16 GB сервер | **128 Hz стрельба!** |
| **GvG 200** | **200** | **6.1 MB** | ✅ Достаточно 64 GB сервер | **80 Hz стрельба!** |
| **GvG 400** | **400** | **7.8 MB** | ✅ Достаточно 64 GB сервер | **60 Hz стрельба!** |
| **Massive War** | **2000** | **82 MB** | ✅ Достаточно 128 GB кластер | **40 Hz стрельба с LOD!** |

**Увеличение памяти связано с:**
- ✅ ShotHistory (история выстрелов для античита)
- ✅ HitDetectionBuffer (валидация попаданий)
- ✅ ShootingState (состояние стрельбы)
- ✅ ActiveProjectiles (активные снаряды)
...
- PvP Small: 128 Hz (как VALORANT) ✅
- GvG: 60-80 Hz (как Battlefield 2042) ✅
- Massive War: 20-40 Hz (как PlanetSide 2) ✅

✅ **Адаптивный тикрейт:** Реализован для оптимизации нагрузки
...
**⚠️ КРИТИЧНО:** Это **НЕ turn-based система!** Все происходит в **реальном времени**:

- ✅ **Выстрелы обрабатываются event-based** с задержкой <16 мс
- ✅ **Состояние стрельбы синхронизируется на 60-128 Hz** (каждые 7.8-16.6 мс)
- ✅ **Попадания валидируются server-side** в реальном времени
- ✅ **Нет пошаговой системы** - все действия происходят одновременно
...
1. ✅ **Память все еще не критична** - даже для 2000 игроков требуется ~82 MB (в 2 раза больше, но приемлемо)
2. ✅ **CPU и Network являются основными ограничениями** - тикрейт и пропускная способность
3. ✅ **Стратегия хранения оптимальна:** In-Memory для активных данных (стрельба!), Redis для кэша, PostgreSQL для персистентности
4. ✅ **Тикрейты соответствуют AAA FPS играм** - 128 Hz для PvP (как VALORANT), 60-80 Hz для GvG (как Battlefield 2042), 20-40 Hz для Massive War (как PlanetSide 2)

**КРИТИЧНО для FPS:**
- ✅ **ShootingState и ShotHistory обязательны** - без них нет валидации стрельбы
- ✅ **HitDetectionBuffer критичен** - server-side validation попаданий
- ✅ **История выстрелов для античита** - минимум 8-32 выстрела в памяти
- ✅ **Задержка выстрела <16 мс** - как в VALORANT/CS:GO
