...
**Исключения**:
- ✅ **Клиент**: Тик сбрасывается при переподключении (`ClientTick = 0` при подключении)
- ✅ **Сервер/Gateway**: Тик НЕ сбрасывается, инкрементируется бесконечно
...
- Время до переполнения: 9,223,372,036,854,775,807 / 1,892,160,000 = **4,875,611,716 лет** ✅
...
- Время до переполнения: 2,147,483,647 / 1,892,160,000 = **~1.13 года** ⚠️
- Это было бы проблемой!

**Но используется `int64`** - проблемы нет ✅

---

## 📦 Размер данных: Почему Move занимает 4 байта?
...
|-----|--------|----------|---------------|
| **float32** | 4 байта | ~7 цифр | ✅ Текущий выбор (оптимально) |
...
## 💡 Потенциальные оптимизации (если нужно)
...
- ✅ Используется `int64` (переполнение через 4.8 миллиарда лет - невозможно)
- ✅ Сбрасывается только при переподключении клиента (ClientTick = 0)
- ✅ Сервер/Gateway: тик инкрементируется бесконечно
...
- ✅ Это стандартный размер для float32 (IEEE 754)
- ✅ Оптимальный баланс: точность vs размер
- ✅ Нельзя уменьшить без потери точности
- ✅ Стандарт для координат в играх

**Рекомендация**: Оставить `float32` - текущий выбор оптимален.

---

## 📊 Сравнение размеров (текущая реализация)

| Поле | Тип | Размер в Protobuf | Оптимизация |
|------|-----|-------------------|-------------|
| Tick | int64 (varint) | 1-9 байт | ✅ Оптимально |
| MoveX | float32 | 5 байт (tag + value) | ✅ Оптимально |
| MoveY | float32 | 5 байт (tag + value) | ✅ Оптимально |
| PlayerID | string | 2-11 байт (tag + length + data) | ⚠️ Можно оптимизировать ("p00000001" → "p1") |
