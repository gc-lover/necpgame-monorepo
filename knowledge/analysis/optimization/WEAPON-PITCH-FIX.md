...
**Проблема**: При интерполяции между двумя снимками состояния (`OldState` и `NewState`) интерполировался не только Yaw,
но и Pitch. Это вызывало:
...
В обработке первого снимка при `World == nullptr` передавался полный `Snapshot.Rotation`, что могло содержать
неправильный Pitch.
...
3. **Сохранение локальных значений**: Pitch и Roll всегда берутся из текущего состояния персонажа, что позволяет
   локальным анимациям работать без конфликтов.

### Почему это важно

- **Pitch управляется камерой**: В UE5 Pitch обычно управляется камерой игрока и не должен синхронизироваться для других
  игроков
- **Анимации оружия**: Локальные анимации оружия (стрельба, перезарядка) управляют Pitch персонажа
- **Визуальная согласованность**: Синхронизация Pitch может вызывать визуальные артефакты, когда локальные анимации
  конфликтуют с сетевыми данными
