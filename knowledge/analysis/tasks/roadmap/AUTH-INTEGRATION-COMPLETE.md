...
- ✅ Создан модуль `server/auth.go` с полной реализацией JWT валидации
- ✅ Использована библиотека `github.com/golang-jwt/jwt/v5` для парсинга токенов
- ✅ Реализована загрузка публичных ключей из Keycloak JWKS endpoint
- ✅ Кэширование публичных ключей с TTL 1 час
- ✅ Проверка issuer, expiration, и подписи токена

### 2. Auth Middleware
- ✅ Создан middleware для проверки JWT токенов
- ✅ Извлечение токена из заголовка `Authorization: Bearer <token>`
- ✅ Добавление claims в контекст запроса
- ✅ Обработка ошибок валидации с правильными HTTP кодами (401)

### 3. Защита API endpoints
- ✅ Все endpoints в `/api/v1/*` защищены middleware
- ✅ Endpoint `/health` доступен без аутентификации
- ✅ Возможность отключения аутентификации через переменную окружения `AUTH_ENABLED=false`
...
- ✅ Добавлена зависимость от сервиса `keycloak`

### 5. Тестирование
- ✅ Компиляция прошла успешно
- ✅ Docker образ собран
- ✅ Health check работает без аутентификации
- ✅ API endpoints правильно отклоняют запросы без токена (401)
- ✅ API endpoints правильно отклоняют запросы с невалидным токеном (401)
