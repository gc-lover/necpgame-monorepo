...
(
    ...
    -- Мета
    difficulty_modifier DECIMAL
(
    3,
    2
) DEFAULT 1.0,
    moral_weight VARCHAR
(
    20
),
    ...
    -- Constraints
    CONSTRAINT fk_branch_quest FOREIGN KEY
(
    quest_id
)
    REFERENCES quests
(
    ...
    CONSTRAINT uq_quest_branch UNIQUE
(
    quest_id,
    branch_id
),
    CONSTRAINT ck_difficulty_modifier CHECK
(
    difficulty_modifier
    >=
    0.1
    ...
    CONSTRAINT ck_moral_weight CHECK
(
    moral_weight
    IN
(
...
-- Комментарии
COMMENT
ON TABLE quest_branches IS 'Ветви/пути прохождения квеста';
COMMENT
ON COLUMN quest_branches.conditions IS 'JSONB: условия активации {flags: [], reputation: {}, choices: []}';
COMMENT
ON COLUMN quest_branches.world_state_changes IS 'JSONB: изменения world state';
COMMENT
ON COLUMN quest_branches.difficulty_modifier IS 'Модификатор сложности: 0.8 = легче, 1.2 = сложнее';
...
INSERT INTO quest_branches (quest_id, branch_id, branch_name, description, conditions, reputation_changes, sets_flags,
                            unlocks_quests, locks_quests, moral_weight)
VALUES
    -- NetWatch path
...
INSERT INTO quest_branches (quest_id, branch_id, branch_name, description, conditions, reputation_changes, sets_flags,
                            unlocks_quests, locks_quests, moral_weight, world_state_changes)
VALUES
    -- Support AI cults
