...
(
    ...
    -- Constraints
    CONSTRAINT fk_dialogue_quest FOREIGN KEY
(
    quest_id
)
    REFERENCES quests
(
    ...
    CONSTRAINT uq_quest_node UNIQUE
(
    quest_id,
    node_id
),
    CONSTRAINT ck_node_type CHECK
(
    node_type
    IN
(
...
COMMENT
ON TABLE dialogue_nodes IS 'Узлы диалогового дерева квеста';
...
(
    ...
    -- Constraints
    CONSTRAINT fk_choice_node FOREIGN KEY
(
    node_id
)
    REFERENCES dialogue_nodes
(
    id
) ON DELETE CASCADE,
    CONSTRAINT uq_node_choice UNIQUE
(
    node_id,
    choice_id
)
...
COMMENT
ON TABLE dialogue_choices IS 'Варианты выбора в узлах диалога';
...
VALUES ('MQ-2020-001', 1, 'marco_fix', 'Marco "Fix" Sanchez',
...
SELECT dn.id,
       'A1',
       '[Принять] Да, помощь не помешает.',
       10,
       '{"Fixers": 10}'::jsonb, '["marco_ally"]'::jsonb, 1
