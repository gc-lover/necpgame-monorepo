...
(
    ...
    -- Метаданные
    set_by_quest VARCHAR
(
    100
),
    set_by_choice VARCHAR
(
    50
),
    ...
    -- Constraints
    CONSTRAINT uq_character_state UNIQUE
(
    character_id,
    state_key
)
...
COMMENT
ON TABLE player_world_state IS 'Персональное состояние мира игрока (видит только он)';
...
(
    ...
    -- Constraints
    CONSTRAINT uq_server_state UNIQUE
(
    server_id,
    state_key
),
...
COMMENT
ON TABLE server_world_state IS 'Коллективное состояние сервера (все игроки видят)';
COMMENT
ON COLUMN server_world_state.threshold_required IS 'Порог голосов для активации (default: 60% онлайна)';
...
(
    ...
    -- Constraints
    CONSTRAINT uq_character_vote UNIQUE
(
    server_id,
    state_key,
    character_id
...
COMMENT
ON TABLE world_state_votes IS 'Голоса игроков за изменение состояния сервера';
COMMENT
ON COLUMN world_state_votes.weight IS 'Вес голоса (reputation-based: 1-10)';
...
(
    ...
    -- Constraints
    CONSTRAINT uq_faction_state UNIQUE
(
    server_id,
    faction_id,
    state_key
...
COMMENT
ON TABLE faction_world_state IS 'Состояние мира по фракциям (видят члены фракции)';
...
(
    ...
    -- Constraints
    CONSTRAINT uq_territory UNIQUE
(
    server_id,
    territory_id
),
    CONSTRAINT ck_control_percentage CHECK
(
    control_percentage
    >=
    0
...
COMMENT
ON TABLE territory_control IS 'Контроль территорий фракциями';
...
VALUES ('server-eu-01', 'fifth_war_winner', '{"faction": "pending"}'::jsonb, 1000, 'pending'),
...
VALUES ('server-eu-01', 'night_city_watson', 'Maelstrom', 70, FALSE),
...
CREATE
OR REPLACE FUNCTION calculate_vote_weight(p_character_id UUID, p_faction_id VARCHAR)
RETURNS INTEGER AS $$
DECLARE
reputation INTEGER;
    base_weight
...
CREATE
OR REPLACE FUNCTION check_vote_threshold(p_server_id VARCHAR, p_state_key VARCHAR)
RETURNS BOOLEAN AS $$
DECLARE
