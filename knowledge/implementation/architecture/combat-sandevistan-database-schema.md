...
Схема базы данных для системы Sandevistan Temporal Overdrive, реализующей эффект замедления времени для конкретного
игрока в MMO без изменения серверного тайминга.
...
Основная таблица активаций Sandevistan. Хранит информацию о фазах активации (подготовка, активная, рекуперация), Action
Priority Budget и уровне перегрева.
...
Таблица состояния Perception Drag для противников. Хранит информацию о клиентском debuff восприятия и серверном debuff
реакции.
...
- `sandevistan_counterplay_logs.counterplay_type`: Допустимые значения: 'emp', 'chrono-jammer', 'hacking', '
  crowd-control'
...
`knowledge/implementation/architecture/combat-sandevistan-temporal-overdrive-architecture.yaml`:
