...
-- Insert all cultures
INSERT INTO cultural_contexts (culture_id, culture_name, region, romance_tempo, romance_style, pda_allowed, pda_level,
                               family_importance, love_phrase)
VALUES ('japanese', 'Japanese', 'asia', 'very_slow', 'reserved', TRUE, 'minimal', 'high', '愛してる'),
...
(
    ...
    trigger_type         VARCHAR(30) CHECK (trigger_type IN (
                                                             ...
                                                             'quest_based', 'random_encounter', 'npc_initiated',
                                                             'player_initiated'
        )),
...
WHERE r.relationship_score >= re.relationship_min
  AND r.relationship_score <= re.relationship_max
  AND re.event_id NOT IN (SELECT unnest(r.completed_events));
...
CREATE
OR REPLACE FUNCTION update_relationship_score(
    p_relationship_id UUID,
    p_change INTEGER
) RETURNS void AS $$
BEGIN
UPDATE relationships
SET relationship_score = GREATEST(-100, LEAST(100, relationship_score + p_change)),
...
CREATE
OR REPLACE FUNCTION calculate_chemistry(
...
SET total_chemistry    = v_total_chemistry,
...
CREATE
OR REPLACE FUNCTION check_event_triggers(
...
:= v_event.triggers;
    ...
    IF
v_relationship.relationship_score < v_event.relationship_min OR
...
END IF;
    ...
    -- Check location
    IF
v_triggers->>'locations' IS NOT NULL THEN
...
END IF;
    ...
    -- Check time
    IF
v_triggers->>'time' IS NOT NULL THEN
...
END IF;
    ...
    IF
v_event.event_id = ANY(v_relationship.completed_events) THEN
        RETURN FALSE;
END IF;
