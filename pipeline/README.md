...
- `scripts/` — PowerShell-валидации и автоматика для переходов.
...
- **Валидация знаний** — перед передачей концептов выполняй `pipeline/scripts/check-knowledge-schema.ps1`.
...
4. Перед сдачей обновляются соответствующие YAML-трекеры и очереди статусов; выполняй `scripts/check-queue-yaml.ps1` после любых правок очереди.
