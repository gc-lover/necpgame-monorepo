# Inventory Management Service - Enterprise-Grade OpenAPI Specification

## 📋 **Назначение**

Этот файл (`main.yaml`) содержит **официальную спецификацию** Inventory Management Service API для NECPGAME проекта.
Спецификация обеспечивает enterprise-grade управление инвентарем игроков с высокой производительностью и надежностью.

## 🎯 **Ключевые Особенности**

### OK **Enterprise-Grade Архитектура**

- Полная совместимость с MMOFPS игровыми доменами
- Оптимизация для высоконагруженных inventory операций
- CQRS паттерны с event-sourced консистентностью
- JWT аутентификация с anti-cheat защитой

### OK **Backend Optimization Hints**

- Struct alignment hints для 30-50% экономии памяти
- Performance targets: P99 <25ms для горячих путей
- Порядок полей: large → small для оптимального выравнивания
- Memory pooling для inventory сессий

### OK **Complete Validation**

- OK Redocly lint: проходит enterprise-grade валидацию
- OK ogen: генерирует production-ready Go код
- OK Go compilation: код компилируется без ошибок
- OK Testing: интеграционные тесты для всех endpoints

### OK **Security-First Approach**

- JWT Bearer authentication с player permissions
- Anti-cheat validation для inventory операций
...
```
proto/openapi/inventory-management-service/
├── main.yaml           # Enterprise-grade OpenAPI спецификация
└── README.md          # Это руководство по использованию
```
...
- `GET /api/v1/inventory/{playerId}` - Получение полного инвентаря
- `PUT /api/v1/inventory/{playerId}` - Обновление инвентаря с валидацией
- `GET /api/v1/inventory/{playerId}/items/{itemId}` - Детали конкретного предмета

### Equipment Management

- `GET /api/v1/inventory/{playerId}/equipment` - Текущая экипировка
- `PUT /api/v1/inventory/{playerId}/equipment` - Смена экипировки (HOT PATH)

### Inventory Operations

- `POST /api/v1/inventory/{playerId}/operations/sort` - Сортировка инвентаря
- `POST /api/v1/inventory/{playerId}/trading/lock` - Блокировка для торговли

### Administrative

- `POST /api/v1/inventory/admin/{playerId}/override` - Admin переопределение
