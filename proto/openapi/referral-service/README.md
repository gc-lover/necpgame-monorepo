# Referral Service - OpenAPI Specification

## Назначение

Этот файл (`main.yaml`) содержит **официальную спецификацию** Referral Service API в NECPGAME проекте. Сервис
управляет реферальными программами, привлечением пользователей и распределением наград в игре Night City.
...
- Enterprise-grade referral management с anti-fraud

### Функциональность

- **Referral Tracking**: Полное отслеживание реферальных ссылок и приглашений
- **Reward Distribution**: Автоматизированный расчет и распределение наград
- **Anti-Fraud Validation**: Проверки целостности рефералов и предотвращение мошенничества
- **Analytics Integration**: Метрики производительности реферальных программ
- **Multi-Level Rewards**: Поддержка многоуровневых реферальных программ
- **Performance Optimized**: Высокопроизводительные операции с рефералами (<25ms P99 latency)
...
- **reward-service**: Распределение наград за рефералы
- **user-profile-service**: Профили пользователей для рефералов
...
- **P99 Latency**: <25ms для операций с рефералами
- **Memory per Instance**: <20KB
- **Concurrent Users**: 100,000+ одновременных операций
- **Throughput**: 35,000+ операций в секунду
...
### Генерация Go кода
```bash
ogen --target ../../services/referral-service-go/pkg/api \
...
**Referral Service гарантирует enterprise-grade управление реферальными программами и совместим со всей экосистемой
NECPGAME AI агентов.**