# Transaction Service - OpenAPI Specification

## 📋 **Назначение**

Transaction Service предоставляет **enterprise-grade API для обработки платежей** в NECPGAME.
Сервис отвечает за все финансовые операции, валидацию транзакций и аудит платежей.

## 🎯 **Функциональность**

- **Payment Processing**: Secure transaction handling with multiple payment methods
- **Transaction Validation**: Fraud detection and transaction integrity checks
- **Financial Operations**: Deposits, withdrawals, transfers, and refunds
- **Audit Trail**: Complete transaction history and compliance logging
- **Multi-Currency Support**: Support for all game currencies and real money
- **Performance Optimized**: High-throughput transaction processing with <20ms P99 latency
...
- **currency-service**: Информация о валютах
- **auth-service**: Аутентификация пользователей
- **user-profile-service**: Профили пользователей

## 📊 **Performance**

- **P99 Latency**: <20ms for transaction operations
- **Memory per Instance**: <30KB per active transaction session
...
```bash
ogen --target ../../services/transaction-service-go/pkg/api \
