...
## ⚠️ Важно: Формат YAML не изменяется!
...
- ✅ Квесты **с** `quest_definition` (полная структура)
- ✅ Квесты **без** `quest_definition` (используются дефолты: `quest_type='side'`, пустые массивы)
- ✅ Разные форматы `quest_id` (`quest-001-willis-tower`, `canon-quest-seoul-gangnam-style`)
- ✅ Datetime объекты в YAML (автоматически конвертируются в ISO format)
...
- ✅ Легко отслеживать изменения конкретного квеста/NPC/диалога
- ✅ Проще откатывать отдельные элементы
- ✅ Версионирование на уровне файла
- ✅ Понятная структура (имя файла отражает путь и версию)

## Поддержка типов контента

Генератор поддерживает:
- ✅ **Quests**: `knowledge/canon/lore/timeline-author/quests/` → `gameplay.quest_definitions`
- ✅ **NPCs**: `knowledge/canon/narrative/npc-lore/` → `narrative.npc_definitions`
- ✅ **Dialogues**: `knowledge/canon/narrative/dialogues/` → `narrative.dialogue_nodes`

**Таблицы созданы:**
- ✅ `gameplay.quest_definitions` (миграция `V1_46__quest_definitions_tables.sql`)
- ✅ `narrative.npc_definitions` (миграция `V1_89__narrative_npc_dialogue_tables.sql`)
- ✅ `narrative.dialogue_nodes` (миграция `V1_89__narrative_npc_dialogue_tables.sql`)
...
✅ **Исправлено!** Генератор автоматически конвертирует datetime в ISO format.
...
✅ **YAML файлы не изменяются** - генератор только читает  
✅ **Все данные сохраняются** - полный YAML в `content_data`  
✅ **Обратная совместимость** - работает с квестами без `quest_definition`  
✅ **Безопасные обновления** - `ON CONFLICT DO UPDATE` не удаляет данные
