...
for service in "${SERVICES[@]}"; do
    ...
    if [ ! -f "$go_mod" ]; then
        echo "WARNING  $service: go.mod not found, skipping"
        SKIPPED=$((SKIPPED + 1))
        continue
    fi
    ...
    if grep -q "github.com/oapi-codegen/runtime" "$go_mod"; then
        echo "OK $service: dependency already exists"
        SKIPPED=$((SKIPPED + 1))
    else
...
echo "  OK Already had dependency: $SKIPPED"
echo "  ➕ Added dependency: $ADDED"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

if [ $ADDED -gt 0 ]; then
    echo "OK Successfully added dependency to $ADDED service(s)!"
fi
