...
foreach ($svc in $services) {
    ...
    try {
        ...
        if ($maxLines -le 500 -and $compiles) {
            Write-Host " ✅ (types=$typesLines | server=$serverLines | spec=$specLines)" -ForegroundColor Green
            $successCount++
            $results += [PSCustomObject]@{Service=$svcName; Status="✅ OK"; MaxLines=$maxLines; Compiles=$true}
        }
        elseif ($maxLines -le 500) {
            Write-Host " ⚠️  Generated but needs handler update (max=$maxLines)" -ForegroundColor Yellow
            $results += [PSCustomObject]@{Service=$svcName; Status="⚠️ Handlers"; MaxLines=$maxLines; Compiles=$false}
        }
        else {
...
Write-Host "⚠️  Needs handlers: $(($results | Where-Object {$_.Status -eq '⚠️ Handlers'}).Count) services" -ForegroundColor Yellow
