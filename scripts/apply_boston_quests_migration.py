#!/usr/bin/env python3
"""
Apply Boston quests migration to database
Issue: #2273
"""
...
def apply_migration():
    """Apply the Boston quests migration"""
    try:
        # Connect to database
        ...
        # Read migration file
        migration_file = Path("infrastructure/liquibase/migrations/data/quests/V006__import_boston_2020_2029_quests.sql")
        ...
        with conn.cursor() as cur:
            ...
            # Check Boston quests using JSON operator
            try:
                cur.execute("SELECT COUNT(*) FROM gameplay.quests WHERE metadata->>'id' LIKE 'canon-quest-boston-%'")
                boston_count = cur.fetchone()[0]
                print(f"[RESULT] Found {boston_count} Boston quests in gameplay.quests table")

                # Show some quest titles
                cur.execute("SELECT metadata->>'id' as quest_id, title FROM gameplay.quests WHERE metadata->>'id' LIKE 'canon-quest-boston-%' ORDER BY metadata->>'id' LIMIT 5")
                quests = cur.fetchall()
                print("[SAMPLE QUESTS]:")
                for quest in quests:
