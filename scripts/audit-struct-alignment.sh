...
check_schema_alignment() {
    ...
    if ! grep -q "BACKEND NOTE" "$file"; then
        echo "⚠️  $file: Missing BACKEND NOTE (struct alignment hint)"
        ((ISSUES_FOUND++))
    fi
    ...
    if grep -B5 "type: boolean" "$file" | grep -q "type: string\|type: array\|format: int64"; then
        echo "⚠️  $file: Boolean fields before large fields (bad alignment)"
        ((ISSUES_FOUND++))
    fi
    
    # Check for int32 BEFORE int64 (suboptimal)
    if grep -B3 "format: int32" "$file" | grep -q "format: int64"; then
        echo "⚠️  $file: int32 before int64 (suboptimal alignment)"
        ((ISSUES_FOUND++))
    fi
}
...
if [[ $ISSUES_FOUND -eq 0 ]]; then
    echo ""
    echo "✅ All OpenAPI specs are properly aligned!"
    exit 0
else
    echo ""
    echo "⚠️  Found $ISSUES_FOUND potential alignment issues"
    echo ""
