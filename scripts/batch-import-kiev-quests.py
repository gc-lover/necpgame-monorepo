...
def batch_import_kiev_quests():
    ...
    # Find all Kiev quest import files
    all_import_files = list(quest_dir.glob("*import.yaml"))
    kiev_quest_files = [
        f for f in all_import_files
        if any(keyword in f.name.lower() for keyword in ['kiev', 'dnieper', 'borsch', 'maidan', 'lavra', 'motherland', 'golden', 'gate', 'sophia', 'cathedral', 'chernobyl', 'khreschatyk', 'cossack'])
    ]

    print(f"Found {len(kiev_quest_files)} Kiev quest import files to process")
    ...
    if failed_count > 0:
        ...
        print("\n[SUCCESS] All Kiev quests imported successfully!")
        return 0

if __name__ == "__main__":
    sys.exit(batch_import_kiev_quests())