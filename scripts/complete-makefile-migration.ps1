# Issue: Complete Makefile migration for remaining services
# Script: Update remaining 8 services + create Makefiles for services without them

$ErrorActionPreference = "Continue"

# Services that need Makefile update
$needsUpdate = @(
    @{Name="client-service"; Router="gorilla-server"},
    @{Name="combat-sessions-service"; Router="chi-server"},
    @{Name="economy-service"; Router="gorilla-server"},
    @{Name="gameplay-service"; Router="gorilla-server"},
    @{Name="projectile-core-service"; Router="chi-server"},
    @{Name="social-player-orders-service"; Router="chi-server"},
    @{Name="social-service"; Router="gorilla-server"},
    @{Name="world-service"; Router="gorilla-server"}
)

# Services without Makefile (need creation)
$needsCreation = @(
    @{Name="combat-actions-service"; Router="chi-server"},
    @{Name="combat-ai-service"; Router="chi-server"},
    @{Name="combat-combos-service"; Router="chi-server"},
    @{Name="combat-turns-service"; Router="chi-server"},
    @{Name="matchmaking"; Router="chi-server"},
    @{Name="quest-rewards-events-service"; Router="gorilla-server"},
    @{Name="realtime-gateway"; Router="chi-server"},
    @{Name="social-chat-commands-service"; Router="gorilla-server"},
    @{Name="social-chat-format-service"; Router="gorilla-server"},
    @{Name="social-chat-moderation-service"; Router="gorilla-server"},
    @{Name="stock-integration-service"; Router="chi-server"},
    @{Name="ws-lobby"; Router="chi-server"}
)

$makefileTemplate = @'
.PHONY: generate-api bundle-api clean verify-api check-deps install-deps generate-types generate-server generate-spec check-file-sizes

SERVICE_NAME := {SERVICE_NAME}
OAPI_CODEGEN := oapi-codegen
REDOCLY_CLI := npx -y @redocly/cli
ROUTER_TYPE := {ROUTER_TYPE}

SPEC_DIR := ../../proto/openapi
API_DIR := pkg/api
SERVICE_SPEC := $(SPEC_DIR)/$(SERVICE_NAME).yaml
BUNDLED_SPEC := $(API_DIR)/$(SERVICE_NAME).bundled.yaml

# Split output files (SOLID compliance)
TYPES_FILE := $(API_DIR)/types.gen.go
SERVER_FILE := $(API_DIR)/server.gen.go
SPEC_FILE := $(API_DIR)/spec.gen.go

check-deps:
	@echo "Checking dependencies..."
	@command -v $(OAPI_CODEGEN) >/dev/null 2>&1 || { echo "❌ oapi-codegen not found"; exit 1; }
	@command -v node >/dev/null 2>&1 || { echo "❌ node not found"; exit 1; }
	@echo "✅ Dependencies installed"

install-deps:
	@go install github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen@latest || true

bundle-api: check-deps
	@echo "Bundling: $(SERVICE_SPEC)"
	@if [ ! -f "$(SERVICE_SPEC)" ]; then echo "❌ Spec not found"; exit 1; fi
	@mkdir -p $(API_DIR)
	@$(REDOCLY_CLI) bundle $(SERVICE_SPEC) -o $(BUNDLED_SPEC) || exit 1
	@echo "✅ Bundled"

generate-types: bundle-api
	@echo "Generating types..."
	@$(OAPI_CODEGEN) -package api -generate types -o $(TYPES_FILE) $(BUNDLED_SPEC) || exit 1
	@wc -l $(TYPES_FILE) | awk '{print "✅ types.gen.go: " $$1 " lines"}'

generate-server: bundle-api
	@echo "Generating server..."
	@$(OAPI_CODEGEN) -package api -generate $(ROUTER_TYPE) -o $(SERVER_FILE) $(BUNDLED_SPEC) || exit 1
	@wc -l $(SERVER_FILE) | awk '{print "✅ server.gen.go: " $$1 " lines"}'

generate-spec: bundle-api
	@echo "Generating spec..."
	@$(OAPI_CODEGEN) -package api -generate spec -o $(SPEC_FILE) $(BUNDLED_SPEC) || exit 1
	@wc -l $(SPEC_FILE) | awk '{print "✅ spec.gen.go: " $$1 " lines"}'

check-file-sizes:
	@echo ""
	@echo "File size check (max 500 lines):"
	@for file in $(TYPES_FILE) $(SERVER_FILE) $(SPEC_FILE); do \
		if [ -f "$$file" ]; then \
			lines=$$(wc -l < "$$file" | tr -d ' '); \
			if [ $$lines -gt 500 ]; then \
				echo "⚠️  $$file: $$lines lines (EXCEEDS)"; \
			else \
				echo "✅ $$file: $$lines lines"; \
			fi; \
		fi; \
	done

generate-api: generate-types generate-server generate-spec check-file-sizes
	@echo ""
	@echo "✅ Generation complete!"

verify-api: check-deps
	@echo "Verifying: $(SERVICE_SPEC)"
	@if [ ! -f "$(SERVICE_SPEC)" ]; then exit 1; fi
	@$(REDOCLY_CLI) lint $(SERVICE_SPEC) || exit 1
	@echo "✅ Valid"

clean:
	@rm -f $(BUNDLED_SPEC) $(TYPES_FILE) $(SERVER_FILE) $(SPEC_FILE)
'@

Write-Host "🔧 Updating/Creating Makefiles..." -ForegroundColor Cyan
Write-Host ""

$updated = 0
$created = 0

# Update existing Makefiles
foreach ($svc in $needsUpdate) {
    $serviceDir = "services\$($svc.Name)-go"
    $makefilePath = "$serviceDir\Makefile"
    
    if (Test-Path $serviceDir) {
        Write-Host "Updating: $($svc.Name)" -ForegroundColor White
        
        $content = $makefileTemplate -replace '\{SERVICE_NAME\}', $svc.Name
        $content = $content -replace '\{ROUTER_TYPE\}', $svc.Router
        
        Set-Content -Path $makefilePath -Value $content -Encoding UTF8
        $updated++
        Write-Host "  ✅ Makefile updated" -ForegroundColor Green
    }
}

# Create new Makefiles
foreach ($svc in $needsCreation) {
    $serviceDir = "services\$($svc.Name)-go"
    $makefilePath = "$serviceDir\Makefile"
    
    if (Test-Path $serviceDir) {
        Write-Host "Creating: $($svc.Name)" -ForegroundColor White
        
        $content = $makefileTemplate -replace '\{SERVICE_NAME\}', $svc.Name
        $content = $content -replace '\{ROUTER_TYPE\}', $svc.Router
        
        Set-Content -Path $makefilePath -Value $content -Encoding UTF8
        $created++
        Write-Host "  ✅ Makefile created" -ForegroundColor Green
    }
}

Write-Host ""
Write-Host "✅ Updated: $updated Makefiles" -ForegroundColor Green
Write-Host "✅ Created: $created Makefiles" -ForegroundColor Green
Write-Host ""
Write-Host "Total services with split generation Makefile: $($updated + $created + 48)" -ForegroundColor Cyan

