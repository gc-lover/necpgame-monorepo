#!/usr/bin/env python3
"""
Convert Baku 2020-2029 canon quests to quest_definition format for database import.

Issue: Convert Baku quests to proper quest_definition YAML format
"""
...
class BakuQuestConverter:
    """
    Convert Baku canon quest files to quest_definition format.
    """

    def __init__(self):
        self.source_dir = Path("knowledge/canon/lore/timeline-author/quests/cis/baku/2020-2029")
        self.output_dir = Path("knowledge/canon/narrative/quests")
        self.output_dir.mkdir(parents=True, exist_ok=True)

    def convert_all_quests(self) -> List[str]:
        """Convert all Baku 2020-2029 quests to quest_definition format."""
        converted_files = []
        ...
        print(f"Found {len(quest_files)} Baku quest files to convert")
    ...
    def convert_quest_file(self, quest_file: Path) -> Path:
        ...
        output_filename = f"baku_{safe_id}_quest_definition.yaml"
    ...
    def generate_rewards(self, content: Dict[str, Any], summary: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'пламенн' in quest_title.lower():
            rewards['items'].append({
                'id': 'flame_tower_model',
                'name': 'Модель пламенных башен',
                'type': 'collectible'
            })
        elif 'янтар' in quest_title.lower():
            rewards['items'].append({
                'id': 'yanar_dag_crystal',
                'name': 'Кристалл Янар Даг',
                'type': 'accessory'
            })
...
def main():
    ...
    converter = BakuQuestConverter()
    converted_files = converter.convert_all_quests()

    print(f"\n[SUCCESS] Successfully converted {len(converted_files)} Baku quests to quest_definition format")
    print("\nConverted files:")
    for file in converted_files:
        print(f"  - {file.name}")

    print("\nReady for database import using batch-import-baku-quests.py script")
    return converted_files
