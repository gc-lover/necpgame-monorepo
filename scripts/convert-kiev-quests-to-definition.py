#!/usr/bin/env python3
"""
Convert Kiev quest YAML files to import format.

Issue: Convert all Kiev quest definitions to database import format
"""
...
def convert_kiev_quests():
    """Convert all Kiev quest files to import format"""
    ...
    # Source directory
    source_dir = base_dir / "knowledge" / "canon" / "lore" / "timeline-author" / "quests" / "cis" / "kiev" / "2020-2029"
    ...
    # Find all YAML files in Kiev directory
    yaml_files = list(source_dir.glob("*.yaml"))

    print(f"Found {len(yaml_files)} Kiev quest files to convert")
    ...
    if failed_count > 0:
        ...
        print("\n[SUCCESS] All Kiev quests converted successfully!")
        return 0


if __name__ == "__main__":
    import sys
    sys.exit(convert_kiev_quests())
