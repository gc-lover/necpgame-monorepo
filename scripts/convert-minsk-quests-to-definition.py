#!/usr/bin/env python3
"""
Convert Minsk 2020-2029 canon quests to quest_definition format for database import.

Issue: Convert 10 Minsk quests to proper quest_definition YAML format
"""
...
class MinskQuestConverter:
    """
    Convert Minsk canon quest files to quest_definition format.
    """

    def __init__(self):
        self.source_dir = Path("knowledge/canon/lore/timeline-author/quests/cis/minsk/2020-2029")
        self.output_dir = Path("knowledge/canon/narrative/quests")
        self.output_dir.mkdir(parents=True, exist_ok=True)

    def convert_all_quests(self) -> List[str]:
        """Convert all Minsk 2020-2029 quests to quest_definition format."""
        converted_files = []
        ...
        print(f"Found {len(quest_files)} Minsk quest files to convert")
    ...
    def convert_quest_file(self, quest_file: Path) -> Path:
        ...
        output_filename = f"minsk_{safe_id}_quest_definition.yaml"
    ...
    def generate_rewards(self, content: Dict[str, Any], summary: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'драник' in quest_title.lower():
            rewards['items'].append({
                'id': 'draniki_recipe',
                'name': 'Рецепт драников',
                'type': 'recipe'
            })
        elif 'метро' in quest_title.lower():
            rewards['items'].append({
                'id': 'metro_pass',
                'name': 'Бесплатный проезд в метро',
                'type': 'accessory'
            })
...
def main():
    ...
    converter = MinskQuestConverter()
    converted_files = converter.convert_all_quests()

    print(f"\n[SUCCESS] Successfully converted {len(converted_files)} Minsk quests to quest_definition format")
    print("\nConverted files:")
    for file in converted_files:
        print(f"  - {file.name}")

    print("\nReady for database import using batch-import-minsk-quests.py script")
    return converted_files
