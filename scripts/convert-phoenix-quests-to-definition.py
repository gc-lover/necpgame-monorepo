#!/usr/bin/env python3
"""
Convert Phoenix quest YAML files to import format.

Issue: Convert all Phoenix quest definitions to database import format
"""
...
def convert_phoenix_quests():
    """Convert all Phoenix quest files to import format"""
    ...
    # Source directory
    source_dir = base_dir / "knowledge" / "canon" / "lore" / "timeline-author" / "quests" / "america" / "phoenix"
    ...
    # Find all YAML files in Phoenix directory
    yaml_files = list(source_dir.rglob("*.yaml"))

    print(f"Found {len(yaml_files)} Phoenix quest files to convert")
    ...
    if failed_count > 0:
        ...
        print("\n[SUCCESS] All Phoenix quests converted successfully!")
        return 0


if __name__ == "__main__":
    import sys
    sys.exit(convert_phoenix_quests())
