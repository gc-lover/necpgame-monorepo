...
foreach ($svc in $services) {
    ...
    if ($Fix) {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  ✅ Fixed: $svc" -ForegroundColor Green
            $fixedCount++
        } else {
            Write-Host "  ⚠️  Issues found (may need manual fix): $svc" -ForegroundColor Yellow
        }
    } else {
        # Check only
        $output = fieldalignment ./... 2>&1
        if ($output -match "struct") {
            Write-Host "  ⚠️  Alignment issues: $svc" -ForegroundColor Yellow
...
if ($Fix) {
    ...
    Write-Host ""
    Write-Host "✅ Struct alignment auto-fix complete!" -ForegroundColor Green
} else {
    Write-Host ""
