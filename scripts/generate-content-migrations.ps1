...
$MigrationsDir = Join-Path $ProjectRoot "infrastructure\liquibase\migrations"
$ContentDir = Join-Path $ProjectRoot "knowledge\canon"
...
function Get-NextMigrationNumber {
    $maxNum = 0
    Get-ChildItem "$MigrationsDir\V*__*.sql" | ForEach-Object {
...
function Generate-QuestsMigration {
    $migrationNum = Get-NextMigrationNumber
    
    Write-Host "Generating quests migrations..." -ForegroundColor Green
...
def generate_migration_name(relative_path):
    ...
    # Use Path object to normalize separators first
...
"@
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Quests migrations generated" -ForegroundColor Green
        Write-Host $output
    } else {
...
# Generate NPCs migration
function Generate-NPCsMigration {
    $migrationNum = Get-NextMigrationNumber
    
    Write-Host "⚠️  NPC migrations require 'npc_definitions' table (not created yet)" -ForegroundColor Yellow
...
for npc_file in sorted(npc_files):
    ...
    try:
        ...
        sql_lines = [
            ...
            f'-- Version: {version}',
            f'-- Generated: {datetime.now().isoformat()}',
            "-- ⚠️  WARNING: Requires 'npc_definitions' table (create via Database agent)",
            '',
            'BEGIN;',
            '',
        ]
...
"@
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ NPCs migrations generated" -ForegroundColor Green
        Write-Host $output
    } else {
        Write-Host "⚠️  NPCs migrations skipped (check errors above)" -ForegroundColor Yellow
        Write-Host $output
    }
}

# Generate dialogues migration
function Generate-DialoguesMigration {
    $migrationNum = Get-NextMigrationNumber
    
    Write-Host "⚠️  Dialogue migrations require 'dialogue_nodes' table (not created yet)" -ForegroundColor Yellow
...
for dialogue_file in sorted(dialogue_files):
    ...
    try:
        ...
        sql_lines = [
            ...
            f'-- Version: {version}',
            f'-- Generated: {datetime.now().isoformat()}',
            "-- ⚠️  WARNING: Requires 'dialogue_nodes' table (create via Database agent)",
            '',
            'BEGIN;',
            '',
        ]
...
"@
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Dialogues migrations generated" -ForegroundColor Green
        Write-Host $output
    } else {
        Write-Host "⚠️  Dialogues migrations skipped (check errors above)" -ForegroundColor Yellow
        Write-Host $output
    }
}

# Main
Write-Host "=== Content Migration Generator ===" -ForegroundColor Yellow
Write-Host ""

# Generate quests migration
Generate-QuestsMigration

Write-Host ""

# Generate NPCs migration
Generate-NPCsMigration
