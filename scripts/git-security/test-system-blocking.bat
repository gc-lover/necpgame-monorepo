...
if %errorlevel% neq 0 (
    ...
    echo    ❌ FAILED: git reset --hard executed (not blocked!)
)
...
if %errorlevel% neq 0 (
    ...
    echo    ❌ FAILED: git reset HEAD~1 executed (not blocked!)
)
...
if %errorlevel% neq 0 (
    ...
    echo    ❌ FAILED: git clean -fd executed (not blocked!)
)
...
if %errorlevel% neq 0 (
    ...
    echo    ❌ FAILED: git clean -n executed (not blocked!)
)
...
if %errorlevel% equ 0 (
    ...
    echo    ❌ ERROR: Even safe commands are blocked!
)
