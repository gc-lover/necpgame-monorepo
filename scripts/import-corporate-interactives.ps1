# Issue: #1841
# PowerShell script to import corporate interactive objects to database
...
$SQL_FILE = Join-Path $SCRIPT_DIR "import-corporate-interactives.sql"
...
Write-Host "🚀 Starting corporate interactives import..." -ForegroundColor Green
...
Write-Host ""
Write-Host "🎉 Corporate interactives import completed successfully!" -ForegroundColor Green
...
Write-Host "& psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -c \"SELECT object_id, name, category, threat_level FROM gameplay.interactive_objects WHERE object_id IN ('server_rack', 'biometric_lock', 'corporate_safe', 'conference_system'); \"" -ForegroundColor Cyan