# Issue: #1843
# PowerShell script to import cyberspace interactive objects to database
...
$SQL_FILE = Join-Path $SCRIPT_DIR "import-cyberspace-interactives.sql"
...
Write-Host "🚀 Starting cyberspace interactives import..." -ForegroundColor Green
...
Write-Host ""
Write-Host "🎉 Cyberspace interactives import completed successfully!" -ForegroundColor Green
...
Write-Host "& psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -c \"SELECT object_id, name, category, threat_level FROM gameplay.interactive_objects WHERE object_id IN ('ice_access_node', 'phantom_archive', 'tournament_arena'); \"" -ForegroundColor Cyan