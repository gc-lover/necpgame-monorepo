#!/usr/bin/env python3
"""
Kiev Part 1 Quests Import Script
Import Kiev Part 1 quest definitions from YAML files to database.
...
class KievPart1QuestImportScript(BaseScript):
    """
    Import Kiev Part 1 quest definitions from YAML files to database.
    """
    ...
    def __init__(self):
        super().__init__(
            name="import-kiev-part1-quests",
            description="Import Kiev Part 1 quest YAML files to database"
        )
    ...
    def add_script_args(self):
        ...
        self.parser.add_argument(
            '--source-dir', '-s',
            type=str,
            default='knowledge/canon/lore/timeline-author/quests/cis/kiev/2020-2029',
...
--changeset kiev-part1-quests:{metadata['id']} runOnChange:true
    ...
    def process_quest_files(self, source_dir: Path, output_dir: Path, force: bool = False) -> List[str]:
        ...
        for yaml_file in yaml_files:
            try:
                ...
                migration_filename = f"{timestamp}__kiev_part1_quest_{metadata['id']}.sql"
...
def main():
    """Entry point"""
    script = KievPart1QuestImportScript()
    script.main()
