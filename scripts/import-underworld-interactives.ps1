# Issue: #1842
# PowerShell script to import underworld interactive objects to database
...
$SQL_FILE = Join-Path $SCRIPT_DIR "import-underworld-interactives.sql"
...
Write-Host "🚀 Starting underworld interactives import..." -ForegroundColor Green
...
Write-Host ""
Write-Host "🎉 Underworld interactives import completed successfully!" -ForegroundColor Green
...
Write-Host "& psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -c \"SELECT object_id, name, category, threat_level FROM gameplay.interactive_objects WHERE object_id IN ('black_market', 'makeshift_lab', 'contraband_tunnel'); \"" -ForegroundColor Cyan