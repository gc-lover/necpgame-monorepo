...
BEGIN
    ...
    IF v_id IS NULL THEN
        ...
        RAISE NOTICE 'Inserted new interactive object: % (%)', p_name, p_object_id;
    ELSE
        -- Update existing record
        UPDATE gameplay.interactive_objects SET
            name = p_name,
        ...
        RAISE NOTICE 'Updated existing interactive object: % (%)', p_name, p_object_id;
    END IF;
...
FROM gameplay.interactive_objects
WHERE object_id IN ('street_terminal', 'ar_billboard', 'access_door', 'delivery_drone', 'garbage_chute', 'security_camera');
