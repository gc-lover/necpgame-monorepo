# Issue: #1844
# PowerShell script to import world common interactive objects to database
...
$SQL_FILE = Join-Path $SCRIPT_DIR "import-world-common-interactives.sql"
...
Write-Host "🚀 Starting world common interactives import..." -ForegroundColor Green
...
Write-Host ""
Write-Host "🎉 World common interactives import completed successfully!" -ForegroundColor Green
...
Write-Host "& psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -c \"SELECT object_id, name, category, threat_level FROM gameplay.interactive_objects WHERE object_id IN ('faction_blockpost', 'comm_relay', 'medical_station', 'logistics_container'); \"" -ForegroundColor Cyan