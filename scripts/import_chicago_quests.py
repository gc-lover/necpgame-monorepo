#!/usr/bin/env python3
"""
Chicago Quests Import Script
Imports Chicago quest data from YAML files to Liquibase YAML format for database insertion.
"""
...
def process_quest_data(quest_data, file_path):
    ...
    try:
        quest = {
            ...
            'location': quest_data.get('location', 'Chicago'),
...
def main():
    """Main function to process Chicago quest files."""
    # Base directory for Chicago quests
    base_dir = Path("knowledge/canon/lore/timeline-author/quests/america/chicago")
    ...
    print(f"Reading Chicago quest files from: {base_dir}")
    ...
    # Process files in chunks of 3 for Part 3
    # Part 3 would be files 7-9 (0-indexed: 6,7,8)
    start_idx = 6
    end_idx = min(9, len(yaml_files))
    ...
    print(f"Successfully parsed {processed_count} Chicago quests")

    # Create Liquibase YAML file
    output_file = "infrastructure/liquibase/data/gameplay/quests/data_quests_chicago_part3_import.yaml"
    os.makedirs(os.path.dirname(output_file), exist_ok=True)

    create_liquibase_yaml(quests_data, output_file)

    print(f"Chicago quests Part 3 import completed successfully!")
