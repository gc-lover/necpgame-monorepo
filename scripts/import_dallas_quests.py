#!/usr/bin/env python3
"""
Import Dallas Quests to Database
...
def parse_dallas_quest(yaml_file):
    """Parse individual Dallas quest from YAML file"""
    try:
...
def create_liquibase_yaml(quests, output_file):
    ...
    for quest in quests:
        ...
        changeset_id = f"quests-dallas-{quest['quest_id']}-{hashlib.md5(str(quest).encode()).hexdigest()[:8]}"

        changeset = {
            'id': changeset_id,
            'author': 'dallas-quests-import',
            'changes': [
                {
                    'insert': {
    ...
    print(f"Imported {len(changesets)} Dallas quests")

def main():
    """Main function"""
    # Input directory with Dallas quests
    input_dir = Path('knowledge/canon/lore/timeline-author/quests/america/dallas/2020-2029')

    # Output file for Liquibase migration
    output_file = Path('infrastructure/liquibase/data/gameplay/quests/data_quests_dallas_2020_2029_import.yaml')

    print(f"Reading Dallas quest files from: {input_dir}")

    # Find all YAML files in the directory
    yaml_files = list(input_dir.glob('*.yaml'))
    print(f"Found {len(yaml_files)} YAML files")

    # Parse all Dallas quests
    dallas_quests = []
    for yaml_file in yaml_files:
        print(f"Processing: {yaml_file.name}")
        quest_data = parse_dallas_quest(yaml_file)
        if quest_data:
            dallas_quests.append(quest_data)

    if not dallas_quests:
        print("No Dallas quests found!")
        return

    print(f"Successfully parsed {len(dallas_quests)} Dallas quests")

    # Create Liquibase YAML
    create_liquibase_yaml(dallas_quests, output_file)

    print("Dallas quests import completed successfully!")
