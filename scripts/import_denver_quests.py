#!/usr/bin/env python3
"""
Import Denver Quests to Database
...
def parse_denver_quest(yaml_file):
    """Parse individual Denver quest from YAML file"""
    try:
...
def create_liquibase_yaml(quests, output_file):
    ...
    for quest in quests:
        ...
        changeset_id = f"quests-denver-{quest['quest_id']}-{hashlib.md5(str(quest).encode()).hexdigest()[:8]}"

        changeset = {
            'id': changeset_id,
            'author': 'denver-quests-import',
            'changes': [
                {
                    'insert': {
    ...
    print(f"Imported {len(changesets)} Denver quests")

def main():
    """Main function"""
    # Input directory with Denver quests
    input_dir = Path('knowledge/canon/lore/timeline-author/quests/america/denver/2020-2029')

    # Output file for Liquibase migration
    output_file = Path('infrastructure/liquibase/data/gameplay/quests/data_quests_denver_2020_2029_import.yaml')

    print(f"Reading Denver quest files from: {input_dir}")

    # Find all YAML files in the directory
    yaml_files = list(input_dir.glob('*.yaml'))
    print(f"Found {len(yaml_files)} YAML files")

    # Parse all Denver quests
    denver_quests = []
    for yaml_file in yaml_files:
        print(f"Processing: {yaml_file.name}")
        quest_data = parse_denver_quest(yaml_file)
        if quest_data:
            denver_quests.append(quest_data)

    if not denver_quests:
        print("No Denver quests found!")
        return

    print(f"Successfully parsed {len(denver_quests)} Denver quests")

    # Create Liquibase YAML
    create_liquibase_yaml(denver_quests, output_file)

    print("Denver quests import completed successfully!")
