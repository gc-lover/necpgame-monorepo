#!/usr/bin/env python3
"""
Import Europe Quests to Database
...
def parse_europe_quest(yaml_file):
    """Parse individual Europe quest from YAML file"""
    try:
...
def create_liquibase_yaml(quests, output_file):
    ...
    for quest in quests:
        ...
        changeset_id = f"quests-europe-{quest['quest_id']}-{hashlib.md5(str(quest).encode()).hexdigest()[:8]}"

        changeset = {
            'id': changeset_id,
            'author': 'europe-quests-import',
            'changes': [
                {
                    'insert': {
    ...
    print(f"Imported {len(changesets)} Europe quests")

def main():
    """Main function"""
    # Input directory with Europe quests
    input_dir = Path('knowledge/canon/lore/timeline-author/quests/europe')

    # Output file for Liquibase migration
    output_file = Path('infrastructure/liquibase/data/gameplay/quests/data_quests_europe_2020_2093_import.yaml')

    print(f"Reading Europe quest files from: {input_dir}")

    # Find all YAML files in the directory recursively
    yaml_files = list(input_dir.glob('**/*.yaml'))
    print(f"Found {len(yaml_files)} YAML files")

    # Parse all Europe quests
    europe_quests = []
    for yaml_file in yaml_files:
        print(f"Processing: {yaml_file.name}")
        quest_data = parse_europe_quest(yaml_file)
        if quest_data:
            europe_quests.append(quest_data)

    if not europe_quests:
        print("No Europe quests found!")
        return

    print(f"Successfully parsed {len(europe_quests)} Europe quests")

    # Create Liquibase YAML
    create_liquibase_yaml(europe_quests, output_file)

    print("Europe quests import completed successfully!")
