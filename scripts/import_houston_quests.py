...
def process_quest_data(quest_data, file_path):
    ...
    try:
        quest = {
            ...
            'location': quest_data.get('location', 'Houston'),
...
def main():
    """Main function to process Houston quest files."""
    # Base directory for Houston quests
    base_dir = Path("knowledge/canon/lore/timeline-author/quests/america/houston")
    ...
    print(f"Reading Houston quest files from: {base_dir}")
    ...
    # Process files in chunks of 3 for Part 2
    # Part 2 would be files 4-6 (0-indexed: 3,4,5)
    start_idx = 3
    end_idx = min(6, len(yaml_files))

    selected_files = yaml_files[start_idx:end_idx]
    print(f"Processing Part 2: files {start_idx+1}-{end_idx} ({len(selected_files)} files)")
    ...
    print(f"Successfully parsed {processed_count} Houston quests")

    # Create Liquibase YAML file
    output_file = "infrastructure/liquibase/data/gameplay/quests/data_quests_houston_part2_import.yaml"
    os.makedirs(os.path.dirname(output_file), exist_ok=True)

    create_liquibase_yaml(quests_data, output_file)

    print(f"Houston quests Part 2 import completed successfully!")
    print(f"Imported {len(quests_data)} quests from Part 2")
