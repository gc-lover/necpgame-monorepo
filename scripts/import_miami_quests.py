...
def process_quest_data(quest_data, file_path):
    ...
    try:
        quest = {
            ...
            'location': quest_data.get('location', 'Miami'),
...
def main():
    """Main function to process Miami quest files."""
    # Base directory for Miami quests
    base_dir = Path("knowledge/canon/lore/timeline-author/quests/america/miami")
    ...
    print(f"Reading Miami quest files from: {base_dir}")
    ...
    # Process files in chunks of 5 for Part 2
    # Part 2 would be files 6-10 (0-indexed: 5,6,7,8,9)
    start_idx = 5
    end_idx = min(10, len(yaml_files))
    ...
    output_file = "infrastructure/liquibase/data/gameplay/quests/data_quests_miami_part2_import.yaml"
    os.makedirs(os.path.dirname(output_file), exist_ok=True)

    create_liquibase_yaml(quests_data, output_file)

    print(f"Miami quests Part 2 import completed successfully!")
