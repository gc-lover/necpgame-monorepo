#!/usr/bin/env python3
"""
Import New York Part 2 Quests to Database
Imports quest data from YAML files to database using Liquibase format.
Imports quests 6-12 from New York 2020-2029 period.
"""
...
def parse_newyork_quest(yaml_file):
    """Parse individual New York quest from YAML file"""
    try:
...
def create_liquibase_yaml(quests, output_file):
    ...
    for quest in quests:
        ...
        changeset_id = f"quests-newyork-part2-{quest['quest_id']}-{hashlib.md5(str(quest).encode()).hexdigest()[:8]}"

        changeset = {
            'id': changeset_id,
            'author': 'newyork-part2-quests-import',
            'changes': [
                {
                    'insert': {
    ...
    print(f"Imported {len(changesets)} New York Part 2 quests")

def main():
    """Main function"""
    # Input directory with New York quests
    input_dir = Path('knowledge/canon/lore/timeline-author/quests/america/new-york/2020-2029')

    # Output file for Liquibase migration
    output_file = Path('infrastructure/liquibase/data/gameplay/quests/data_quests_newyork_part2_import.yaml')

    print(f"Reading New York quest files from: {input_dir}")

    # Find all YAML files in the directory
    yaml_files = list(input_dir.glob('*.yaml'))
    print(f"Found {len(yaml_files)} YAML files")

    # Parse all New York quests
    newyork_quests = []
    for yaml_file in yaml_files:
        print(f"Processing: {yaml_file.name}")
        quest_data = parse_newyork_quest(yaml_file)
        if quest_data:
            newyork_quests.append(quest_data)

    if not newyork_quests:
        print("No New York quests found!")
        return

    print(f"Successfully parsed {len(newyork_quests)} New York quests")

    # Create Liquibase YAML
    create_liquibase_yaml(newyork_quests, output_file)

    print("New York Part 2 quests import completed successfully!")
