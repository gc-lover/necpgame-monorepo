#!/usr/bin/env python3
"""
Phoenix Part 3 Quests Import Script
Imports Phoenix quest data from YAML files to Liquibase YAML format for database insertion.
...
def process_quest_data(quest_data, file_path):
    ...
    try:
        quest = {
            ...
            'location': quest_data.get('location', 'Phoenix'),
...
def create_liquibase_yaml(quests_data, output_file):
    ...
    liquibase_data = {
        'databaseChangeLog': [
            {
                'changeSet': {
                    'id': 'insert-phoenix-part3-quests',
...
def main():
    """Main function to process Phoenix quest files."""
    # Base directory for Phoenix quests
    base_dir = Path("knowledge/canon/lore/timeline-author/quests/america/phoenix")
    ...
    print(f"Reading Phoenix quest files from: {base_dir}")
    ...
    # Process Part 3: files 11-15 (0-indexed: 10,11,12,13,14)
    start_idx = 10
    end_idx = min(15, len(yaml_files))
    ...
    print(f"Successfully parsed {processed_count} Phoenix quests")

    # Create Liquibase YAML file
    output_file = "infrastructure/liquibase/data/gameplay/quests/data_quests_phoenix_part3_import.yaml"
    os.makedirs(os.path.dirname(output_file), exist_ok=True)

    create_liquibase_yaml(quests_data, output_file)

    print("Phoenix quests Part 3 import completed successfully!")
