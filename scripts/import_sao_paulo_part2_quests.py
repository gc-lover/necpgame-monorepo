...
def create_liquibase_yaml(quests, output_file):
    ...
    changeset_id = f"data_quests_sao_paulo_part2_import_{datetime.now().strftime('%Y%m%d%H%M%S')}"
...
def main():
    ...
    # Process only Part 2 quest files (files 6-11, indices 5-10)
    quest_files = sorted([f for f in input_dir.glob('quest-*.yaml')])[5:11]
