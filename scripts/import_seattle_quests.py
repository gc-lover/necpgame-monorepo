#!/usr/bin/env python3
"""
Import Seattle Quests to Database
...
def parse_seattle_quest(yaml_file):
    """Parse individual Seattle quest from YAML file"""
    try:
...
def create_liquibase_yaml(quests, output_file):
    ...
    for quest in quests:
        ...
        changeset_id = f"quests-seattle-{quest['quest_id']}-{hashlib.md5(str(quest).encode()).hexdigest()[:8]}"

        changeset = {
            'id': changeset_id,
            'author': 'seattle-quests-import',
            'changes': [
                {
                    'insert': {
    ...
    print(f"Imported {len(changesets)} Seattle quests")

def main():
    """Main function"""
    # Input directory with Seattle quests
    input_dir = Path('knowledge/canon/lore/timeline-author/quests/america/seattle/2020-2029')

    # Output file for Liquibase migration
    output_file = Path('infrastructure/liquibase/data/gameplay/quests/data_quests_seattle_2020_2029_import.yaml')

    print(f"Reading Seattle quest files from: {input_dir}")

    # Find all YAML files in the directory
    yaml_files = list(input_dir.glob('*.yaml'))
    print(f"Found {len(yaml_files)} YAML files")

    # Parse all Seattle quests
    seattle_quests = []
    for yaml_file in yaml_files:
        print(f"Processing: {yaml_file.name}")
        quest_data = parse_seattle_quest(yaml_file)
        if quest_data:
            seattle_quests.append(quest_data)

    if not seattle_quests:
        print("No Seattle quests found!")
        return

    print(f"Successfully parsed {len(seattle_quests)} Seattle quests")

    # Create Liquibase YAML
    create_liquibase_yaml(seattle_quests, output_file)

    print("Seattle quests import completed successfully!")
