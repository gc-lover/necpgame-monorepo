#!/usr/bin/env python3
"""
Shanghai Part 2 Quests Import Script
Imports Shanghai quest data from YAML files to Liquibase YAML format for database insertion.
...
def process_quest_data(quest_data, file_path):
    ...
    try:
        ...
        quest = {
            ...
            'location': quest_def.get('location', 'shanghai'),
...
def create_liquibase_yaml(quests, output_file):
    ...
    changeset_id = f"data_quests_shanghai_part2_import_{datetime.now().strftime('%Y%m%d%H%M%S')}"
        ...
        'databaseChangeLog': [
            {
                'changeSet': {
                    'id': changeset_id,
                    'author': 'shanghai_quests_importer',
                    'changes': [
                        {
                            'insert': {
                                'tableName': 'quests',
...
def main():
    """Main function to import Shanghai Part 2 quests."""
    input_dir = Path('knowledge/canon/lore/timeline-author/quests/asia/shanghai/2020-2029')
    output_file = Path('infrastructure/liquibase/data/gameplay/quests/data_quests_shanghai_part2_import.yaml')

    quests = []

    # Process Part 2 quest files (files 6-10, indices 5-9, but only available files)
    quest_files = sorted([f for f in input_dir.glob('quest-*.yaml')])[5:9]  # Files 6-8,10
    ...
    if quests:
        ...
        print(f"Successfully processed {len(quests)} Shanghai Part 2 quests")
    else:
