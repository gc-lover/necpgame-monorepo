...
def parse_side_quest(yaml_file):
    """Parse individual side quest from YAML file"""
    try:
        ...
        quest_type = quest_def.get('quest_type', 'main')
        if quest_type != 'side':
            return None
        ...
        # Check if period is within 2020-2077
        start_year = int(period.split('-')[0])
        if start_year >= 2020 and start_year <= 2077:
            quest_data = {
                'id': str(uuid.uuid4()),
                'quest_id': quest_id,
                'title': title,
...
def create_liquibase_yaml(quests, output_file):
    ...
    for quest in quests:
        ...
        changeset_id = f"quests-side-{quest['quest_id']}-{hashlib.md5(str(quest).encode()).hexdigest()[:8]}"

        changeset = {
            'id': changeset_id,
            'author': 'side-quests-import',
            'changes': [
                {
                    'insert': {
    ...
    print(f"Imported {len(changesets)} side quests")

def main():
    """Main function"""
    input_base_dir = Path('knowledge/canon/lore/timeline-author/quests')

    print(f"Reading side quests from: {input_base_dir}")
    ...
    for yaml_file in yaml_files:
        ...
        if processed_count % 100 == 0:
            print(f"Processed {processed_count} files, found {side_count} side quests")

        quest_data = parse_side_quest(yaml_file)
        if quest_data:
            side_quests.append(quest_data)
            side_count += 1

    print(f"Successfully parsed {len(side_quests)} side quests from 2020-2077 period")

    if len(side_quests) < 20:
        print(f"Warning: Found only {len(side_quests)} side quests, expected around 20")

    output_file = Path('infrastructure/liquibase/data/gameplay/quests/data_quests_side_2020_2077_import.yaml')
    create_liquibase_yaml(side_quests, output_file)

    print("Side quests import completed successfully!")
