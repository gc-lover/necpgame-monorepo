...
def parse_start_content_quest(yaml_file):
    """Parse individual start content quest from YAML file"""
    try:
        ...
        # Check if this is a start content quest (level_min = 1)
            ...
            rewards = [
                {
                    'type': 'experience',
                    'amount': 500,
                    'description': 'Опыт за выполнение стартового квеста'
                },
                {
                    'type': 'currency',
                    'amount': 200,
...
def create_liquibase_yaml(quests, output_file):
    ...
    for quest in quests:
        ...
        changeset_id = f"quests-start-content-{quest['quest_id']}-{hashlib.md5(str(quest).encode()).hexdigest()[:8]}"

        changeset = {
            'id': changeset_id,
            'author': 'start-content-quests-import',
            'changes': [
                {
                    'insert': {
    ...
    print(f"Imported {len(changesets)} start content quests")

def main():
    """Main function"""
    input_base_dir = Path('knowledge/canon/lore/timeline-author/quests')

    print(f"Reading start content quests (level_min=1) from: {input_base_dir}")

    start_content_quests = []
    ...
    processed_count = 0
    start_content_count = 0

    for yaml_file in yaml_files:
        processed_count += 1
        if processed_count % 200 == 0:
            print(f"Processed {processed_count} files, found {start_content_count} start content quests")

        quest_data = parse_start_content_quest(yaml_file)
        if quest_data:
            start_content_quests.append(quest_data)
            start_content_count += 1

            # Limit to 15 quests as per task description
            if start_content_count >= 15:
                break

    print(f"Successfully parsed {len(start_content_quests)} start content quests")

    output_file = Path('infrastructure/liquibase/data/gameplay/quests/data_quests_start_content_import.yaml')
    create_liquibase_yaml(start_content_quests, output_file)

    print("Start content quests import completed successfully!")
