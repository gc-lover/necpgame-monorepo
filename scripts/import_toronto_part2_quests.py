#!/usr/bin/env python3
"""
Toronto Part 2 Quests Import Script
...
def create_liquibase_yaml(quests, output_file):
    ...
    changeset_id = f"data_quests_toronto_part2_import_{datetime.now().strftime('%Y%m%d%H%M%S')}"
...
def main():
    """Main function to import Toronto Part 2 quests."""
    input_dir = Path('knowledge/canon/lore/timeline-author/quests/america/toronto/2020-2029')
    output_file = Path('infrastructure/liquibase/data/gameplay/quests/data_quests_toronto_part2_import.yaml')
    ...
    if quests:
        ...
        print(f"Successfully processed {len(quests)} Toronto Part 2 quests")
    else:
