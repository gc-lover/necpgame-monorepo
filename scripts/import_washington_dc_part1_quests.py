#!/usr/bin/env python3
"""
Washington DC Part 1 Quests Import Script
...
def create_liquibase_yaml(quests, output_file):
    ...
    changeset_id = f"data_quests_washington_dc_part1_import_{datetime.now().strftime('%Y%m%d%H%M%S')}"
...
def main():
    """Main function to import Washington DC Part 1 quests."""
    input_dir = Path('knowledge/canon/lore/timeline-author/quests/america/washington-dc/2020-2029')
    output_file = Path('infrastructure/liquibase/data/gameplay/quests/data_quests_washington_dc_part1_import.yaml')
    ...
    if quests:
        ...
        print(f"Successfully processed {len(quests)} Washington DC Part 1 quests")
    else:
