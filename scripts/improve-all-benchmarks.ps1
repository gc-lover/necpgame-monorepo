# Issue: Improve all benchmarks automatically
...
Write-Host "Improving and testing all benchmarks..." -ForegroundColor Cyan
Write-Host ""
...
foreach ($ServiceDir in $ServiceDirs) {
    ...
    try {
        ...
        # Находим методы handlers (первые 3)
        $HandlerMethods = @()
        $AllMatches = [regex]::Matches($HandlersContent, 'func \(h \*Handlers\) (\w+)\([^)]+\)')
        foreach ($match in $AllMatches) {
            $methodName = $match.Groups[1].Value
            if ($methodName -notin $HandlerMethods -and $methodName -ne "NewHandlers") {
                $HandlerMethods += $methodName
            }
        }
        ...
        foreach ($Method in $MethodsToBench) {
            ...
            } elseif ($NewHandlersSig -match "service.*Service") {
            ...
            if ($Method -match "Get|List") {
                ...
                if ($Method -match "Get.*ID|Get.*Id") {
                    $BenchContent += "		ID: uuid.New(),`n"
                } elseif ($Method -match "Get.*Player") {
                    $BenchContent += "		PlayerID: uuid.New(),`n"
                } elseif ($Method -match "Get.*Character") {
                    $BenchContent += "		CharacterID: uuid.New(),`n"
                } elseif ($Method -match "Get.*Quest") {
                    $BenchContent += "		QuestID: uuid.New(),`n"
                }
                $BenchContent += "	}`n`n"
            ...
            } elseif ($Method -match "Create|Update|Post|Start|Complete") {
                $BenchContent += "	req := &api.${Method}Request{`n"
                $BenchContent += "		// TODO: Fill request fields based on API spec`n"
                $BenchContent += "	}`n`n"
                $BenchContent += "	b.ReportAllocs()`n"
                $BenchContent += "	b.ResetTimer()`n`n"
                $BenchContent += "	for i := 0; i < b.N; i++ {`n"
        ...
        $TestResult = go test -run=^$ -bench=. -benchmem ./server 2>&1
        Pop-Location
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "    Needs manual fix" -ForegroundColor Yellow
            $Fixed++
        }
