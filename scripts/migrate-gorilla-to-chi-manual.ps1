...
foreach ($svc in $gorillaServices) {
    ...
    if ($serverFiles.Count -eq 0) {
        Write-Host "  ⚠️ No server files found" -ForegroundColor Yellow
        continue
    }
    ...
    if ($filesUpdated -gt 0) {
        ...
        if (Test-Path $makefilePath) {
            ...
            Write-Host "  ✅ Makefile updated to chi-server" -ForegroundColor Green
        }
        ...
        if ($bundled) {
            ...
            Pop-Location
            Write-Host "  ✅ server.gen.go regenerated with chi" -ForegroundColor Green
        }
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  ✅ Compiles successfully!" -ForegroundColor Green
            $migrated++
        } else {
...
Write-Host "✅ Successfully migrated: $migrated" -ForegroundColor Green
if ($failed -gt 0) {
