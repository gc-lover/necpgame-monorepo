...
foreach ($svc in $services) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $results += [PSCustomObject]@{Service=$svc; Status="⚠️ Manual"}
        }
    }
    catch {
...
Write-Host "⚠️  Needs manual: $(($results | Where-Object {$_.Status -eq '⚠️ Manual'}).Count) services" -ForegroundColor Yellow
