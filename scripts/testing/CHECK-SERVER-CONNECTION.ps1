    ...
    if ($metrics.StatusCode -eq 200) {
        ...
        if ($metricsContent -match 'active_server_connection\s+(\d+)') {
            ...
            if ($serverConnection -eq "1") {
                Write-Host "  ✅ Server подключен к Gateway!" -ForegroundColor Green
            } else {
                Write-Host "  ⚠️  Server НЕ подключен (active_server_connection = $serverConnection)" -ForegroundColor Yellow
                Write-Host "     Убедитесь, что UE5 Server запущен в PIE режиме" -ForegroundColor White
            }
        } else {
            Write-Host "  ⚠️  Метрика active_server_connection не найдена" -ForegroundColor Yellow
        }
...
try {
    ...
    if ($wsTest) {
        Write-Host "  ✅ WebSocket порт 18080 доступен" -ForegroundColor Green
    } else {
        Write-Host "  ❌ WebSocket порт 18080 недоступен" -ForegroundColor Red
    }
} catch {
    Write-Host "  ⚠️  Не удалось проверить WebSocket порт" -ForegroundColor Yellow
}
