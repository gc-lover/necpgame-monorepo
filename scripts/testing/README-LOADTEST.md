...
- OK Все клиенты подключаются успешно (Failed Connections = 0)
- OK PlayerInput отправляется с заданной частотой (±5%)
- OK GameState получается регулярно
- OK Нет ошибок соединения
- OK Размеры сообщений соответствуют ожидаемым (PlayerInput ~47 байт)
