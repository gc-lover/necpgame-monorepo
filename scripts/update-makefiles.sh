...
for service_info in "${SERVICES[@]}"; do
    ...
    if [ ! -f "$makefile_path" ]; then
        echo "⚠️  $service_dir: Makefile not found, skipping"
        SKIPPED=$((SKIPPED + 1))
        continue
    fi
    
    if grep -q "check-deps" "$makefile_path"; then
        echo "✅ $service_dir: already has improved Makefile"
        SKIPPED=$((SKIPPED + 1))
    else
...
verify-api: check-deps
	...
	@\$(REDOCLY_CLI) lint \$(SERVICE_SPEC) || { echo "⚠️  OpenAPI spec validation failed"; exit 1; }
...
if [ $UPDATED -gt 0 ]; then
    echo "✅ Successfully updated $UPDATED Makefile(s)!"
fi
