...
for service_info in "${SERVICES[@]}"; do
    ...
    if [ ! -f "../../proto/openapi/$service_name.yaml" ]; then
        echo "  WARNING  OpenAPI spec not found: proto/openapi/$service_name.yaml"
        SKIPPED=$((SKIPPED + 1))
        echo ""
        continue
    fi
    ...
    if make verify-api > /dev/null 2>&1; then
        ...
        echo "  WARNING  OpenAPI spec validation failed (might be OK)"
        SKIPPED=$((SKIPPED + 1))
    fi
...
if [ $FAILED -eq 0 ]; then
    echo "OK All services are properly configured!"
    exit 0
else
