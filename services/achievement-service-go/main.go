...
func main() {
	...
	logger := log.New(os.Stdout, "[achievement] ", log.LstdFlags)

	// PERFORMANCE: Initialize service with memory pooling
	svc := server.NewAchievementService()
	...
	go func() {
		logger.Printf("Starting achievement service on :8100 (GOGC=%s)", os.Getenv("GOGC"))
