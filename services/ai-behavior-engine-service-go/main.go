...
import (
	...
	"necpgame/services/ai-behavior-engine-service-go/server"
)
...
func main() {
	...
	slog.Info("Starting AI Behavior Engine Service",
		"addr", *addr,
		"version", "1.0.0",
	)
	...
	// Initialize service
	service := server.NewAiBehaviorEngineService(repo)

	// Initialize middleware
	middleware := server.NewMiddleware()

	// Create API server
	apiServer := server.NewAiBehaviorEngineServer(service)
...
func setupRouter(apiServer *server.AiBehaviorEngineServer, middleware *server.Middleware) http.Handler {
