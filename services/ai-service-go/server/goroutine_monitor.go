...
func (gm *GoroutineMonitor) Start() {
	...
	go func() {
		ticker := time.NewTicker(15 * time.Second) // OPTIMIZATION: Check every 15s for AI processing
		defer ticker.Stop()
...
func (gm *GoroutineMonitor) checkGoroutines() {
	...
	if stats.IsOverLimit {
		// OPTIMIZATION: Issue #1972 - Alert on excessive goroutines for AI
		...
		}).Warn("AI goroutine count exceeded maximum allowed - possible AI processing overload")

		// Force garbage collection as emergency measure for AI
		runtime.GC()
		runtime.ForceGC()
	} else {
		...
		gm.logger.WithFields(logrus.Fields{
			"current_goroutines": stats.CurrentCount,
			"max_allowed":        stats.MaxAllowed,
		}).Debug("AI goroutine count within limits")
	}
}
