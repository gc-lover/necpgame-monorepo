...
import (
	...
	"auth-expansion-domain-service-go/server"
)
...
func main() {
	...
	logger := log.New(os.Stdout, "[auth-expansion-domain] ", log.LstdFlags)
	...
	svc := server.NewAuthexpansiondomainService()
	...
	go func() {
		logger.Printf("Starting auth-expansion-domain service on :8080 (GOGC=%s)", os.Getenv("GOGC"))
