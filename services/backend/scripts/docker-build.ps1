...
if (-not (Test-Path "../API-SWAGGER")) {
    Write-Host "WARNING  Директория API-SWAGGER не найдена. Убедитесь, что спецификации доступны перед сборкой." -ForegroundColor Yellow
}
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "`nOK Образ успешно собран: $fullTag" -ForegroundColor Green
