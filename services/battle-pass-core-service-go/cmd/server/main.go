...
func main() {
	var cfg Config
	err := envconfig.Process("BATTLE_PASS_CORE", &cfg)
	if err != nil {
	...
	log.Info().Msg("Starting Battle Pass Core Service")
	...
	svc := server.NewBattlePassCoreService(pool)
	...
	log.Info().Msg("Shutting down Battle Pass Core Service...")
	...
	log.Info().Msg("Battle Pass Core Service shut down gracefully")
}
