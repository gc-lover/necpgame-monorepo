...
func main() {
	...
	logger.Info("Battle Pass Service (Go) starting...")

	addr := getEnv("ADDR", "0.0.0.0:8093")
	...
	battlePassService, err := server.NewBattlePassService(dbURL, redisURL)
	if err != nil {
		logger.WithError(err).Fatal("Failed to initialize battle pass service")
	}
	...
	httpServer := server.NewHTTPServer(addr, battlePassService, jwtValidator, authEnabled)
