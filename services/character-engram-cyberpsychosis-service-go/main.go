...
import (
	...
	"github.com/necpgame/character-engram-cyberpsychosis-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Character Engram Cyberpsychosis Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6703")
	...
	monitor := server.NewGoroutineMonitor(150, logger) // Max 150 goroutines for character-engram-cyberpsychosis service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Character Engram Cyberpsychosis Service (Go) stopped.")
}
