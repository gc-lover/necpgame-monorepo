...
import (
	...
	"github.com/necpgame/character-engram-security-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Character Engram Security Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6389")
	...
	monitor := server.NewGoroutineMonitor(150, logger) // Max 150 goroutines for character-engram-security service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Character Engram Security Service (Go) stopped.")
}
