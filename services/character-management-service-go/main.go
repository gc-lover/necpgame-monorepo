...
import (
	...
	"github.com/NECPGAME/character-management-service-go/server"

	_ "github.com/jackc/pgx/v5/stdlib"
)

// main является точкой входа для character-management-service
func main() {
	...
	logger.Info("Starting character-management-service", zap.String("version", "1.0.0"))
	...
	// Создаем HTTP сервер
	httpServer := server.NewHTTPServer(logger, db, redisClient, kafkaWriter, &server.Config{
		JWTSecret:   config.JWTSecret,
		ServerPort:  config.ServerPort,
		OAuthConfig: server.OAuthConfig(config.OAuthConfig),
	})
...
func loadConfig() (*Config, error) {
	config := &Config{
		DatabaseURL: getEnv("DATABASE_URL", "postgres://user:password@localhost:5432/character_db?sslmode=disable"),
