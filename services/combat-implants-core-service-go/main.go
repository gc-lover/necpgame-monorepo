...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/combat-implants-core-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Combat Implants Core Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6431")
	...
	monitor := server.NewGoroutineMonitor(150, logger) // Max 150 goroutines for combat-implants-core service
	go monitor.Start()
	defer monitor.Stop()
