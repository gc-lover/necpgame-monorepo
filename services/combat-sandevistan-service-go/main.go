...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/combat-sandevistan-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Combat Sandevistan Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6488")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for combat-sandevistan service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Combat Sandevistan Service (Go) stopped.")
}
