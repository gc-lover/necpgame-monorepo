# Combat Turns Service - ogen Migration Summary
...
**Service:** `combat-turns-service-go`  
...
Implemented 4 turn management operations:
1. OK `GetCurrentTurn` - Get current turn
2. OK `GetTurnOrder` - Get turn order
3. OK `NextTurn` - Advance to next turn
4. OK `SkipTurn` - Skip current turn
...
└── service.go        - Business logic (4 methods)
```

### 5. **Fixes Applied**
- OK Fixed service.go return types (CurrentTurn, TurnOrder)
