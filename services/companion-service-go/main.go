...
func main() {
	...
	companionService, err := server.NewCompanionService(dbURL, redisURL)
	if err != nil {
		logger.WithError(err).Fatal("Failed to initialize companion service")
	}

	httpServer := server.NewHTTPServer(addr, companionService)
