...
func main() {
	...
	addr := getEnv("ADDR", "0.0.0.0:8117")

	httpServer := server.NewHTTPServer(addr, logger)

	// OPTIMIZATION: Issue #1584 - Start pprof server for profiling
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6285")
	...
	monitor := server.NewGoroutineMonitor(150, logger) // Max 150 goroutines for cosmetic service
	go monitor.Start()
	defer monitor.Stop()
