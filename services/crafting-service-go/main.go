// Crafting Service - Enterprise-grade crafting system for Cyberpunk RPG
// Issue: #2228
// PERFORMANCE: Optimized for 2000+ RPS with struct alignment and memory pooling
...
import (
	...
	"crafting-service-go/server"
)

func main() {
	// PERFORMANCE: Optimize GC for high-throughput crafting operations
	if os.Getenv("GOGC") == "" {
		os.Setenv("GOGC", "40") // Lower GC threshold for crafting service
	}
	...
	go func() {
		logger.Printf("Starting crafting-service on :8210 (GOGC=%s)", os.Getenv("GOGC"))
