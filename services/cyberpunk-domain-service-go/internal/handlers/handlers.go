...
// HTTP handlers for cyberpunk domain service
...
type Handlers struct {
	service *service.CyberpunkDomainService
	logger  *zap.Logger
	config  *config.Config
}

// NewHandlers creates new handlers
func NewHandlers(svc *service.CyberpunkDomainService, logger *zap.Logger, cfg *config.Config) *Handlers {
	return &Handlers{
		service: svc,
		logger:  logger,
		config:  cfg,
...
// DomainHealthCheck handles cyberpunk domain specific health check
...
func (h *Handlers) writeJSONResponse(w http.ResponseWriter, statusCode int, data interface{}) {
	...
	w.Header().Set("ETag", "\"-api-v1-cyberpunk-domain-health-v1.0\"")
