...
func main() {
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6560")
	...
	log.Println("✅ Partition management completed successfully")
}
...
func (pm *PartitionManager) ManagePartitions(ctx context.Context) error {
	...
	for _, config := range tables {
		...
		log.Printf("✅ %s.%s partitions managed successfully", config.Schema, config.Table)
	}
...
func (pm *PartitionManager) createFuturePartitions(ctx context.Context, config PartitionConfig) error {
	...
	for i := 0; i <= config.FutureDays; i++ {
		...
		log.Printf("  ✅ Created partition: %s (%s to %s)", partitionName, startDate, endDate)
	}
...
func (pm *PartitionManager) dropOldPartitions(ctx context.Context, config PartitionConfig) error {
	...
	if droppedCount > 0 {
		log.Printf("  ✅ Dropped %d old partitions", droppedCount)
	}
