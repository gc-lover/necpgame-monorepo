...
import (
	...
	"economy-service-service-go/server"
)
...
func main() {
	...
	logger := log.New(os.Stdout, "[economy-service] ", log.LstdFlags)
	...
	svc := server.NewEconomyserviceService()
	...
	go func() {
		logger.Printf("Starting economy-service service on :8080 (GOGC=%s)", os.Getenv("GOGC"))
