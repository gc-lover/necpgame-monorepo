...
// FriendServiceHealthCheckParams is parameters of friendServiceHealthCheck operation.
type FriendServiceHealthCheckParams struct {
	AcceptEncoding OptFriendServiceHealthCheckAcceptEncoding `json:",omitempty,omitzero"`
}

func unpackFriendServiceHealthCheckParams(packed middleware.Parameters) (params FriendServiceHealthCheckParams) {
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.AcceptEncoding = v.(OptFriendServiceHealthCheckAcceptEncoding)
		}
	}
	return params
}

func decodeFriendServiceHealthCheckParams(args [0]string, argsEscaped bool, r *http.Request) (params FriendServiceHealthCheckParams, _ error) {
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotAcceptEncodingVal FriendServiceHealthCheckAcceptEncoding
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotAcceptEncodingVal = FriendServiceHealthCheckAcceptEncoding(c)
					return nil
				}(); err != nil {
					return err
				}
