/**
 * Generated by orval v7.13.2 🍺
 * Do not edit manually.
 * Auction House Mechanics API
 * Контракты economy-service для прикладных механик аукционного дома NECPGAME.
Покрывает создание лотов, ставки, buyout, автопродление, отмену, завершение,
комиссионные правила, уведомления и различия с Player Market.

 * OpenAPI spec version: 1.0.0
 */
import type { AuctionCreateRequestDurationHours } from './auctionCreateRequestDurationHours'
import type { AutoBidSettings } from './autoBidSettings'

export interface AuctionCreateRequest {
  sellerId: string
  itemId: string
  /** @minimum 1 */
  quantity: number
  /** @minimum 0 */
  startingBid: number
  /** @nullable */
  buyoutPrice?: number | null
  durationHours: AuctionCreateRequestDurationHours
  autoBid?: AutoBidSettings
  /** Токен проверки anti-fraud сервиса */
  antiFraudToken?: string
}
