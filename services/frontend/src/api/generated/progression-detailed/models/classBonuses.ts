/**
 * Generated by orval v7.13.2 🍺
 * Do not edit manually.
 * Progression Detailed API
 * API для детальной системы прогрессии.

Функционал:
- 9 Attributes (STR, DEX, CON, INT, WIS, CHA, TECH, COOL, LUCK)
- Attributes matrix (стартовые бонусы по классам)
- Attributes caps & growth
- Skills-to-classes mapping
- Skills-to-items mapping
- Skills-to-implants mapping
- Synergy bonuses

Источники: .BRAIN/02-gameplay/progression/ (4 documents v1.0.0)

 * OpenAPI spec version: 1.0.0
 */
import type { ClassBonusesAttributeBonuses } from './classBonusesAttributeBonuses'
import type { ClassBonusesSkillBonuses } from './classBonusesSkillBonuses'

export interface ClassBonuses {
  class_id?: string
  class_name?: string
  /** Бонусы к атрибутам */
  attribute_bonuses?: ClassBonusesAttributeBonuses
  /** Бонусы к навыкам */
  skill_bonuses?: ClassBonusesSkillBonuses
  special_abilities?: string[]
  starting_perks?: string[]
}
