/**
 * Generated by orval v7.13.2 🍺
 * Do not edit manually.
 * Stealth API
 * API для системы скрытности.

Источник: .BRAIN/02-gameplay/combat/combat-stealth.md v1.1.0

Механики:
- Уровни видимости: Hidden → Detected
- Звук и свет: шум от действий, освещение, тени
- Действия: Crouch, Takedown, Hide Body, Distraction
- Враги: зрение, слух, поиск, тревога
- Импланты: оптический камуфляж, глушители звука, радар врагов
- Интеграция: с D&D checks, netrunning, социальными навыками

Вдохновение: Deus Ex, Dishonored, Hitman

 * OpenAPI spec version: 1.0.0
 */
import type { StealthStatusStealthLevel } from './stealthStatusStealthLevel'
import type { StealthStatusEnemiesAware } from './stealthStatusEnemiesAware'

export interface StealthStatus {
  character_id?: string
  /** Текущий уровень скрытности */
  stealth_level?: StealthStatusStealthLevel
  /**
   * Видимость персонажа (%)
   * @minimum 0
   * @maximum 100
   */
  visibility?: number
  /**
   * Уровень шума (%)
   * @minimum 0
   * @maximum 100
   */
  noise_level?: number
  /**
   * Освещенность (%)
   * @minimum 0
   * @maximum 100
   */
  light_exposure?: number
  enemies_aware?: StealthStatusEnemiesAware
  /** Активные эффекты (optical_camo, sound_dampener) */
  active_effects?: string[]
}
