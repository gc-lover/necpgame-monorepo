/**
 * Generated by orval v7.13.2 🍺
 * Do not edit manually.
 * Combat Weapons API
 * API для детальной системы оружия в стиле Cyberpunk.

Источник концепции: .BRAIN/02-gameplay/combat/combat-weapon-classes-detailed.md v1.0.0

Особенности:
- 7 классов оружия: Pistols, Assault Rifles, Shotguns, Sniper Rifles, SMG, LMG, Melee
- 80+ моделей оружия из лора Cyberpunk
- 5 брендов: Arasaka, Militech, Kang Tao, Budget Arms, Constitutional Arms
- Weapon Mastery: 5 ranks (Novice → Legend)
- Weapon Mods система
- Exotic/Legendary оружие
- Cyberware оружие: Mantis Blades, Gorilla Arms, Monowire

ПРИМЕЧАНИЕ: Модели данных вынесены в weapons/weapons-models.yaml
для соблюдения лимита 400 строк на файл.

 * OpenAPI spec version: 1.0.0
 */
import type { WeaponDetailsWeaponClass } from './weaponDetailsWeaponClass'
import type { WeaponDetailsRarity } from './weaponDetailsRarity'
import type { WeaponStats } from './weaponStats'
import type { WeaponDetailsSpecialAbilitiesItem } from './weaponDetailsSpecialAbilitiesItem'
import type { WeaponDetailsRequirements } from './weaponDetailsRequirements'

export interface WeaponDetails {
  id: string
  name: string
  description?: string
  /** Лор из Cyberpunk 2077 */
  lore?: string
  weapon_class: WeaponDetailsWeaponClass
  /** Подкласс (например, revolver, semi-auto pistol) */
  subclass?: string
  brand?: string
  rarity: WeaponDetailsRarity
  stats: WeaponStats
  /** Специальные способности оружия */
  special_abilities?: WeaponDetailsSpecialAbilitiesItem[]
  /** Количество слотов для модов */
  mod_slots?: number
  compatible_mods?: string[]
  requirements?: WeaponDetailsRequirements
}
