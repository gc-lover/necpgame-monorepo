...
import type { ListCharacters200 } from '../../../api/generated/auth/models'
...
interface CharacterCreationFormProps {
  ...
  onSuccess?: (characterName?: string, characterId?: string) => void
...
export function CharacterCreationForm({ name, gender, onNameChange, onGenderChange, onSuccess, onCancel, onSubmitRef, onCanCreateRef, onIsPendingRef, onSelectedInfoChange }: CharacterCreationFormProps) {
  ...
  const handleSubmit = () => {
    ...
    if (!classesData?.classes || classesData.classes.length === 0) {
      showError('Ошибка: данные классов не загружены. Пожалуйста, обновите страницу.')
      return
    }
    
    if (!originsData?.origins || originsData.origins.length === 0) {
      showError('Ошибка: данные происхождений не загружены. Пожалуйста, обновите страницу.')
      return
    }
    ...
    if (!classData) {
      showError('Ошибка: класс не найден. Пожалуйста, выберите класс снова.')
      return
    }
    
    if (!originData) {
      showError('Ошибка: происхождение не найдено. Пожалуйста, выберите происхождение снова.')
      return
    }
    ...
    createCharacter(
      { data: request },
      {
        onSuccess: async (response) => {
          ...
          // Оптимистично обновляем кэш списка персонажей
          // Получаем текущие данные
          const currentData = queryClient.getQueryData<ListCharacters200>(['/characters'])
          
          // Оптимистично обновляем кэш списка персонажей
          // Используем только поля, которые есть в GameCharacterSummary
          if (currentData && response.character) {
            const characterSummary = {
              id: response.character.id,
              name: response.character.name,
              class: response.character.class,
              level: response.character.level,
              faction_name: response.character.faction_name || null,
              city_name: response.character.city_name,
              last_login: response.character.last_login || null,
            }
            
            queryClient.setQueryData<ListCharacters200>(['/characters'], {
              ...currentData,
              characters: [...(currentData.characters || []), characterSummary],
              current_count: (currentData.current_count || 0) + 1,
            })
          }
          
          // Затем обновляем данные с сервера (в фоне, не блокируя UI)
          queryClient.invalidateQueries({ queryKey: ['/characters'] })
          
          // Вызываем callback с именем и ID персонажа для анимации
          onSuccess?.(response.character?.name, response.character?.id)
        },
        onError: (err: any) => {
          ...
          console.error(`Ошибка создания персонажа (${err.response?.status || 'N/A'}): ${errorMessage}`)
          showError(`Ошибка создания персонажа (${err.response?.status || 'N/A'}): ${errorMessage}`)
        },
      }
    )
  }
