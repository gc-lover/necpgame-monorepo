...
func (r *PlayerSkillRater) CalculatePlayerRating(ctx context.Context, playerID uuid.UUID) (*api.PlayerSkillRatingResponse, error) {
	// Try cache first
	cacheKey := fmt.Sprintf("player_rating:%s", playerID.String())
	if cached, err := r.redis.Get(ctx, cacheKey).Result(); err == nil {
		var rating api.PlayerSkillRatingResponse
...
func (r *PlayerSkillRater) calculateRatingFromStats(ctx context.Context, playerID uuid.UUID) (*api.PlayerSkillRatingResponse, error) {
	...
	recommendation := r.getRecommendedDifficulty(skillRating)

	rating := &api.PlayerSkillRatingResponse{
		PlayerID:         playerID,
...
// getRecommendedDifficulty returns the recommended difficulty level
func (r *PlayerSkillRater) getRecommendedDifficulty(skillRating float64) *api.DifficultyLevel {
	if skillRating < 20 {
		return &api.DifficultyLevel{
			Level:      api.DifficultyLevelLevel("very_easy"),
			Name:       "Very Easy",
			RatingRange: &api.DifficultyLevelRatingRange{
				Min: 0,
				Max: 20,
			},
		}
...
func (r *PlayerSkillRater) getDefaultRating() *api.PlayerSkillRatingResponse {
	return &api.PlayerSkillRatingResponse{
		PlayerID:        uuid.Nil, // Will be set by caller
		...
		LastUpdated:     time.Now(),
		Recommendation: &api.DifficultyLevel{
			Level:      "normal",
			Name:       "Normal",
			RatingRange: &api.DifficultyLevelRatingRange{
				Min: 40,
