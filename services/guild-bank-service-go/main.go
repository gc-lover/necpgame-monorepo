...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/guild-bank-service-go/server"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Guild Bank Service starting...")
	...
	monitor := server.NewGoroutineMonitor(150, logger) // Max 150 goroutines for guild-bank service
	go monitor.Start()
	defer monitor.Stop()
	...
	// Start server
	go func() {
		logger.WithField("addr", addr).Info("Starting Guild Bank Service")
