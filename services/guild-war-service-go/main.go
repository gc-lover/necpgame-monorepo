...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/guild-war-service-go/server"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Guild War Service starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6826")
	...
	monitor := server.NewGoroutineMonitor(100, logger) // Max 100 goroutines for guild-war service
	go monitor.Start()
	defer monitor.Stop()
	...
	// Start server
	go func() {
		logger.WithField("addr", addr).Info("Starting Guild War Service")
