...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/hacking-core-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Hacking Core Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6728")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for hacking-core service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Hacking Core Service (Go) stopped.")
}
