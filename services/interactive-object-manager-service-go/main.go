...
import (
	...
	"necpgame/services/interactive-object-manager-service-go/server"
)
...
func main() {
	...
	// Initialize service
	service := server.NewInteractiveObjectManagerService(repo)

	// Create API handler
	apiHandler := server.NewInteractiveObjectManagerServer(service)

	// Setup HTTP server
	httpServer := server.NewHTTPServer(*addr, apiHandler)
