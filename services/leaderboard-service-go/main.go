...
func main() {
	...
	metricsAddr := getEnv("METRICS_ADDR", ":9087")
	...
	leaderboardRepo := server.NewLeaderboardRepository(db)
	eventBus := server.NewRedisEventBus(redisClient)
	leaderboardService := server.NewLeaderboardService(leaderboardRepo, logger, eventBus)
	...
	httpServer := server.NewHTTPServer(addr, leaderboardService, jwtValidator, authEnabled)
