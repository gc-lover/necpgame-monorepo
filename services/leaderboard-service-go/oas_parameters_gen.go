...
// LeaderboardServiceHealthCheckParams is parameters of leaderboardServiceHealthCheck operation.
type LeaderboardServiceHealthCheckParams struct {
	AcceptEncoding OptLeaderboardServiceHealthCheckAcceptEncoding `json:",omitempty,omitzero"`
}

func unpackLeaderboardServiceHealthCheckParams(packed middleware.Parameters) (params LeaderboardServiceHealthCheckParams) {
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.AcceptEncoding = v.(OptLeaderboardServiceHealthCheckAcceptEncoding)
		}
	}
	return params
}

func decodeLeaderboardServiceHealthCheckParams(args [0]string, argsEscaped bool, r *http.Request) (params LeaderboardServiceHealthCheckParams, _ error) {
	h := uri.NewHeaderDecoder(r.Header)
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotAcceptEncodingVal LeaderboardServiceHealthCheckAcceptEncoding
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotAcceptEncodingVal = LeaderboardServiceHealthCheckAcceptEncoding(c)
					return nil
				}(); err != nil {
					return err
				}
