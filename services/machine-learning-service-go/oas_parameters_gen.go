...
// MachineLearningServiceHealthCheckParams is parameters of machineLearningServiceHealthCheck operation.
type MachineLearningServiceHealthCheckParams struct {
	AcceptEncoding OptMachineLearningServiceHealthCheckAcceptEncoding `json:",omitempty,omitzero"`
}

func unpackMachineLearningServiceHealthCheckParams(packed middleware.Parameters) (params MachineLearningServiceHealthCheckParams) {
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.AcceptEncoding = v.(OptMachineLearningServiceHealthCheckAcceptEncoding)
		}
	}
	return params
}

func decodeMachineLearningServiceHealthCheckParams(args [0]string, argsEscaped bool, r *http.Request) (params MachineLearningServiceHealthCheckParams, _ error) {
	h := uri.NewHeaderDecoder(r.Header)
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotAcceptEncodingVal MachineLearningServiceHealthCheckAcceptEncoding
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotAcceptEncodingVal = MachineLearningServiceHealthCheckAcceptEncoding(c)
					return nil
				}(); err != nil {
					return err
				}
