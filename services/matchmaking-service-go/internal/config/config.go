...
func Load() (*Config, error) {
	...
	cfg.Database = DatabaseConfig{
		...
		User:         getEnv("DB_USER", "matchmaking"),
	...
	cfg.Matchmaking = MatchmakingConfig{
		QueueTimeout:         time.Duration(getEnvAsInt("MATCHMAKING_QUEUE_TIMEOUT_MIN", 30)) * time.Minute,
		MatchmakingInterval:  time.Duration(getEnvAsInt("MATCHMAKING_INTERVAL_SEC", 5)) * time.Second,
		MaxPlayersPerMatch:   getEnvAsInt("MATCHMAKING_MAX_PLAYERS_PER_MATCH", 10),
		DefaultEstimatedWait: time.Duration(getEnvAsInt("MATCHMAKING_DEFAULT_ESTIMATED_WAIT_SEC", 60)) * time.Second,
	}
