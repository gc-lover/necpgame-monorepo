...
// ModerationServiceHealthCheckParams is parameters of moderationServiceHealthCheck operation.
type ModerationServiceHealthCheckParams struct {
	AcceptEncoding OptModerationServiceHealthCheckAcceptEncoding `json:",omitempty,omitzero"`
}

func unpackModerationServiceHealthCheckParams(packed middleware.Parameters) (params ModerationServiceHealthCheckParams) {
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.AcceptEncoding = v.(OptModerationServiceHealthCheckAcceptEncoding)
		}
	}
	return params
}

func decodeModerationServiceHealthCheckParams(args [0]string, argsEscaped bool, r *http.Request) (params ModerationServiceHealthCheckParams, _ error) {
	h := uri.NewHeaderDecoder(r.Header)
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotAcceptEncodingVal ModerationServiceHealthCheckAcceptEncoding
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotAcceptEncodingVal = ModerationServiceHealthCheckAcceptEncoding(c)
					return nil
				}(); err != nil {
					return err
				}
