...
5. **WARNING Целевой микросервис** (бекенд всегда реализуется через микросервис)
6. **WARNING Целевой фронтенд-модуль** (для фронтенда)
7. **WARNING Требуемые UI компоненты** (@shared/ui, @shared/forms)
