...
// PetServiceHealthCheckParams is parameters of petServiceHealthCheck operation.
type PetServiceHealthCheckParams struct {
	AcceptEncoding OptPetServiceHealthCheckAcceptEncoding `json:",omitempty,omitzero"`
}

func unpackPetServiceHealthCheckParams(packed middleware.Parameters) (params PetServiceHealthCheckParams) {
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.AcceptEncoding = v.(OptPetServiceHealthCheckAcceptEncoding)
		}
	}
	return params
}

func decodePetServiceHealthCheckParams(args [0]string, argsEscaped bool, r *http.Request) (params PetServiceHealthCheckParams, _ error) {
	h := uri.NewHeaderDecoder(r.Header)
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotAcceptEncodingVal PetServiceHealthCheckAcceptEncoding
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotAcceptEncodingVal = PetServiceHealthCheckAcceptEncoding(c)
					return nil
				}(); err != nil {
					return err
				}
