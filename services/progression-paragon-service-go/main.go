...
import (
	...
	"github.com/necpgame/progression-paragon-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Progression Paragon Service (Go) starting...")

	addr := getEnv("ADDR", "0.0.0.0:8084")
	metricsAddr := getEnv("METRICS_ADDR", ":9084")
	...
	paragonService, err := server.NewParagonService(dbURL, redisURL)
	if err != nil {
	...
	monitor := server.NewGoroutineMonitor(150, logger) // Max 150 goroutines for progression-paragon service
	go monitor.Start()
	defer monitor.Stop()
