...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/quest-rewards-events-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Quest Rewards Events Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6314")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for quest-rewards-events service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Quest Rewards Events Service (Go) stopped.")
}
