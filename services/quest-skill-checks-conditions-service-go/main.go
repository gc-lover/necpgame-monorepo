...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/quest-skill-checks-conditions-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Quest Skill Checks Conditions Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6882")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for quest-skill-checks-conditions service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Quest Skill Checks Conditions Service (Go) stopped.")
}
