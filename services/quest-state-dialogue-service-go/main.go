...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/quest-state-dialogue-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Quest State Dialogue Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6812")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for quest-state-dialogue service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Quest State Dialogue Service (Go) stopped.")
}
