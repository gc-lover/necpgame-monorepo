...
func main() {
	...
	for numClients := config.StartClients; numClients <= config.MaxClients; numClients += config.StepSize {
		...
		if success {
			...
			fmt.Printf("OK PASSED: %d clients - Error rate: %.2f%%, Throughput: %.2f msg/s (%.1f%% of expected %.0f msg/s)\n",
			...
			if result.ErrorRate >= config.ErrorThreshold*2 || throughputRatio < 80.0 {
				if result.ErrorRate >= config.ErrorThreshold*2 {
					fmt.Printf("WARNING  Error rate too high (%.2f%%), stopping limit search.\n", result.ErrorRate)
				} else {
					fmt.Printf("WARNING  Throughput too low (%.1f%% of expected), stopping limit search.\n", throughputRatio)
				}
				break
			}
		}
