...
func main() {
	...
	logger.Info("Referral Service (Go) starting...")
	...
	referralRepo := server.NewReferralRepository(db)
	eventBus := server.NewRedisEventBus(redisClient)
	referralService := server.NewReferralService(referralRepo, logger, eventBus)
	...
	httpServer := server.NewHTTPServer(addr, referralService, jwtValidator, authEnabled)
