...
import (
	...
	"github.com/necpgame/seasonal-challenges-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Seasonal Challenges Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6820")
	...
	monitor := server.NewGoroutineMonitor(150, logger) // Max 150 goroutines for seasonal-challenges service
	go monitor.Start()
	defer monitor.Stop()
