...
import (
	...
	"security-service-go/internal/handlers"
	"security-service-go/internal/middleware/security"
	"security-service-go/internal/monitoring"
...
func main() {
	...
	logger.Info().Str("version", "1.0.0").Str("environment", cfg.Environment).Msg("Starting Security Service")

	// Initialize database connections
	ctx := context.Background()

	// PostgreSQL connection with connection pooling (50 max connections for security operations)
	...
	// Redis connection for JWT blacklists and session management
	...
	// Initialize handlers
	securityHandlers := handlers.NewSecurityHandlers(authService, dbLayer, logger)
	...
	r.Get("/health", securityHandlers.HealthCheck)
	r.Get("/ready", securityHandlers.ReadinessCheck)
	...
	oasServer, err := api.NewServer(securityHandlers, nil)
	if err != nil {
