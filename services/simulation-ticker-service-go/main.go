...
func main() {
	...
	// Create Kafka producer with context timeout
	...
	message := kafka.Message{
		...
		Headers: []kafka.Header{
			{Key: "event_type", Value: []byte(tickEvent.EventType)},
			{Key: "source", Value: []byte(tickEvent.Source)},
			{Key: "version", Value: []byte(tickEvent.Version)},
			{Key: "tick_type", Value: []byte(*tickType)},
		},
	}
