...
// SkillServiceHealthCheckParams is parameters of skillServiceHealthCheck operation.
type SkillServiceHealthCheckParams struct {
	AcceptEncoding OptSkillServiceHealthCheckAcceptEncoding `json:",omitempty,omitzero"`
}

func unpackSkillServiceHealthCheckParams(packed middleware.Parameters) (params SkillServiceHealthCheckParams) {
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.AcceptEncoding = v.(OptSkillServiceHealthCheckAcceptEncoding)
		}
	}
	return params
}

func decodeSkillServiceHealthCheckParams(args [0]string, argsEscaped bool, r *http.Request) (params SkillServiceHealthCheckParams, _ error) {
	h := uri.NewHeaderDecoder(r.Header)
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotAcceptEncodingVal SkillServiceHealthCheckAcceptEncoding
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotAcceptEncodingVal = SkillServiceHealthCheckAcceptEncoding(c)
					return nil
				}(); err != nil {
					return err
				}
