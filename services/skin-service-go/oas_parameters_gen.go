...
// SkinServiceHealthCheckParams is parameters of skinServiceHealthCheck operation.
type SkinServiceHealthCheckParams struct {
	AcceptEncoding OptSkinServiceHealthCheckAcceptEncoding `json:",omitempty,omitzero"`
}

func unpackSkinServiceHealthCheckParams(packed middleware.Parameters) (params SkinServiceHealthCheckParams) {
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.AcceptEncoding = v.(OptSkinServiceHealthCheckAcceptEncoding)
		}
	}
	return params
}

func decodeSkinServiceHealthCheckParams(args [0]string, argsEscaped bool, r *http.Request) (params SkinServiceHealthCheckParams, _ error) {
	h := uri.NewHeaderDecoder(r.Header)
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotAcceptEncodingVal SkinServiceHealthCheckAcceptEncoding
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotAcceptEncodingVal = SkinServiceHealthCheckAcceptEncoding(c)
					return nil
				}(); err != nil {
					return err
				}
