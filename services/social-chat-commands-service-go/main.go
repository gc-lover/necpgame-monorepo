...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/social-chat-commands-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Social Chat Commands Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6552")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for social-chat-commands service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Social Chat Commands Service (Go) stopped.")
}
