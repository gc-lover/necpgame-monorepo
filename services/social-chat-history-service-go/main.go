...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/social-chat-history-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Social Chat History Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6714")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for social-chat-history service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Social Chat History Service (Go) stopped.")
}
