...
func main() {
	...
	logger.Info("Social Chat Messages Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6653")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for social-chat-messages service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Social Chat Messages Service (Go) stopped.")
}
