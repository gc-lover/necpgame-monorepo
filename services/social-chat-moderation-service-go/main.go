...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/social-chat-moderation-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Social Chat Moderation Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6502")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for social-chat-moderation service
	go monitor.Start()
	defer monitor.Stop()
	...
	logger.Info("Social Chat Moderation Service (Go) stopped.")
}
