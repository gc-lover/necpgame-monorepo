...
// Config holds all configuration for the Social Domain Service
type Config struct {
	ServerAddr       string
	DatabaseURL      string
	RedisURL         string
...
func Load() (*Config, error) {
	...
	// Parse timeouts (optimized for MMOFPS social interactions)
	...
	config := &Config{
		...
		DatabaseURL:      getEnv("DATABASE_URL", "postgres://user:password@localhost:5432/social_system?sslmode=disable"),
