...
import (
	...
	"specialized-domain/character-service-go-service-go/server"
)
...
func main() {
	...
	logger := log.New(os.Stdout, "[specialized-domain/character-service-go] ", log.LstdFlags)
	...
	svc := server.NewSpecializeddomain/CharacterservicegoService()
	...
	go func() {
		logger.Printf("Starting specialized-domain/character-service-go service on :8080 (GOGC=%s)", os.Getenv("GOGC"))
