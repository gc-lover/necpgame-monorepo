...
// SpectatorServiceHealthCheckParams is parameters of spectatorServiceHealthCheck operation.
type SpectatorServiceHealthCheckParams struct {
	AcceptEncoding OptSpectatorServiceHealthCheckAcceptEncoding `json:",omitempty,omitzero"`
}

func unpackSpectatorServiceHealthCheckParams(packed middleware.Parameters) (params SpectatorServiceHealthCheckParams) {
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.AcceptEncoding = v.(OptSpectatorServiceHealthCheckAcceptEncoding)
		}
	}
	return params
}

func decodeSpectatorServiceHealthCheckParams(args [0]string, argsEscaped bool, r *http.Request) (params SpectatorServiceHealthCheckParams, _ error) {
	h := uri.NewHeaderDecoder(r.Header)
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotAcceptEncodingVal SpectatorServiceHealthCheckAcceptEncoding
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotAcceptEncodingVal = SpectatorServiceHealthCheckAcceptEncoding(c)
					return nil
				}(); err != nil {
					return err
				}
