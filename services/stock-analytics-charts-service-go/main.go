...
func main() {
	...
	logger.Info("Stock Analytics Charts Service starting...")

	addr := getEnv("ADDR", "0.0.0.0:8080")
	metricsAddr := getEnv("METRICS_ADDR", ":9090")

	repository := server.NewInMemoryRepository(logger)
	chartsService := server.NewChartsService(repository, logger)
	handlers := server.NewHandlers(chartsService, logger)
