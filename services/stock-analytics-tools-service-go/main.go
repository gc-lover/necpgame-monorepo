...
func main() {
	...
	logger.Info("Stock Analytics Tools Service starting...")

	addr := getEnv("ADDR", "0.0.0.0:8080")
	metricsAddr := getEnv("METRICS_ADDR", ":9090")

	repository := server.NewInMemoryRepository(logger)
	toolsService := server.NewToolsService(repository, logger)
	handlers := server.NewHandlers(toolsService, logger)
