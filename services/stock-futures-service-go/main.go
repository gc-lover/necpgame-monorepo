...
import (
	...
	"github.com/necpgame/stock-futures-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Stock Futures Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6623")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for stock-futures service
	go monitor.Start()
	defer monitor.Stop()
