...
import (
	...
	"github.com/necpgame/stock-margin-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Stock Margin Service (Go) starting...")

	addr := getEnv("ADDR", "0.0.0.0:8098")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6453")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for stock-margin service
	go monitor.Start()
	defer monitor.Stop()
