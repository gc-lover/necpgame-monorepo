...
import (
	...
	"github.com/necpgame/stock-options-service-go/server"
	"github.com/prometheus/client_golang/prometheus/promhttp"
)

func main() {
	logger := server.GetLogger()
	logger.Info("Stock Options Service (Go) starting...")
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6758")
	...
	monitor := server.NewGoroutineMonitor(200, logger) // Max 200 goroutines for stock-options service
	go monitor.Start()
	defer monitor.Stop()
