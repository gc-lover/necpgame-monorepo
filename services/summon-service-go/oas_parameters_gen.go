...
// SummonServiceHealthCheckParams is parameters of summonServiceHealthCheck operation.
type SummonServiceHealthCheckParams struct {
	AcceptEncoding OptSummonServiceHealthCheckAcceptEncoding `json:",omitempty,omitzero"`
}

func unpackSummonServiceHealthCheckParams(packed middleware.Parameters) (params SummonServiceHealthCheckParams) {
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.AcceptEncoding = v.(OptSummonServiceHealthCheckAcceptEncoding)
		}
	}
	return params
}

func decodeSummonServiceHealthCheckParams(args [0]string, argsEscaped bool, r *http.Request) (params SummonServiceHealthCheckParams, _ error) {
	h := uri.NewHeaderDecoder(r.Header)
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotAcceptEncodingVal SummonServiceHealthCheckAcceptEncoding
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotAcceptEncodingVal = SummonServiceHealthCheckAcceptEncoding(c)
					return nil
				}(); err != nil {
					return err
				}
