...
import (
	...
	"system-domain-service-go/server"
)
...
func main() {
	...
	logger := log.New(os.Stdout, "[system-domain] ", log.LstdFlags)
	...
	svc := server.NewSystemdomainService()
	...
	go func() {
		logger.Printf("Starting system-domain service on :8080 (GOGC=%s)", os.Getenv("GOGC"))
