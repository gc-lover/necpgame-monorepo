...
// TerritoryServiceHealthCheckParams is parameters of territoryServiceHealthCheck operation.
type TerritoryServiceHealthCheckParams struct {
	AcceptEncoding OptTerritoryServiceHealthCheckAcceptEncoding `json:",omitempty,omitzero"`
}

func unpackTerritoryServiceHealthCheckParams(packed middleware.Parameters) (params TerritoryServiceHealthCheckParams) {
	{
		key := middleware.ParameterKey{
		...
		if v, ok := packed[key]; ok {
			params.AcceptEncoding = v.(OptTerritoryServiceHealthCheckAcceptEncoding)
		}
	}
	return params
}

func decodeTerritoryServiceHealthCheckParams(args [0]string, argsEscaped bool, r *http.Request) (params TerritoryServiceHealthCheckParams, _ error) {
	h := uri.NewHeaderDecoder(r.Header)
	...
	if err := func() error {
		...
		if err := h.HasParam(cfg); err == nil {
			if err := h.DecodeParam(cfg, func(d uri.Decoder) error {
				var paramsDotAcceptEncodingVal TerritoryServiceHealthCheckAcceptEncoding
				if err := func() error {
					val, err := d.DecodeValue()
					if err != nil {
						return err
					...
					paramsDotAcceptEncodingVal = TerritoryServiceHealthCheckAcceptEncoding(c)
					return nil
				}(); err != nil {
					return err
				}
