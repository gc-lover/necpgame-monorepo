...
// OPTIMIZATION: Issue #2166 - Runtime Goroutine Monitoring for tournament stability
...
// OPTIMIZATION: Issue #2166 - Memory-aligned struct
...
func (gm *GoroutineMonitor) Start() {
	...
	go func() {
		ticker := time.NewTicker(30 * time.Second) // OPTIMIZATION: Check every 30s for tournament goroutines
		defer ticker.Stop()
...
func (gm *GoroutineMonitor) checkGoroutines() {
	...
	if stats.IsOverLimit {
		// OPTIMIZATION: Issue #2166 - Alert on excessive goroutines for tournament
		...
		}).Warn("Tournament goroutine count exceeded maximum allowed - possible tournament processing overload")

		// Force garbage collection as emergency measure for tournament
		runtime.GC()
		runtime.ForceGC()
	} else {
		...
		gm.logger.WithFields(logrus.Fields{
			"current_goroutines": stats.CurrentCount,
			"max_allowed":        stats.MaxAllowed,
		}).Debug("Tournament goroutine count within limits")
	}
}
