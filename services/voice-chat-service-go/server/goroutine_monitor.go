...
// OPTIMIZATION: Issue #1585 - Runtime Goroutine Monitoring for voice chat stability
...
// OPTIMIZATION: Issue #2030 - Memory-aligned struct
...
func (gm *GoroutineMonitor) Start() {
	...
	go func() {
		ticker := time.NewTicker(30 * time.Second) // OPTIMIZATION: Check every 30s for voice connections
		defer ticker.Stop()
...
func (gm *GoroutineMonitor) checkGoroutines() {
	...
	if stats.IsOverLimit {
		// OPTIMIZATION: Issue #2030 - Alert on excessive goroutines for voice chat
		...
		}).Warn("Voice chat goroutine count exceeded maximum allowed - possible connection overload")

		// Force garbage collection as emergency measure for voice chat
		runtime.GC()
		runtime.ForceGC()
	} else {
		...
		gm.logger.WithFields(logrus.Fields{
			"current_goroutines": stats.CurrentCount,
			"max_allowed":        stats.MaxAllowed,
		}).Debug("Voice chat goroutine count within limits")
	}
}
