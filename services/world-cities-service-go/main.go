...
import (
	...
	"necpgame/services/world-cities-service-go/server"
)

// main является точкой входа для world-cities-service
func main() {
	...
	logger.Info("Starting world-cities-service", zap.String("version", "1.0.0"))
	...
	// Создаем HTTP сервер
	server := server.NewWorldCitiesServer(logger, db, config.JWTSecret)
...
func loadConfig() (*Config, error) {
	config := &Config{
		...
		ServerPort:  getEnvAsInt("SERVER_PORT", 8089),
	}
