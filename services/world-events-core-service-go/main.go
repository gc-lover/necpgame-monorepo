...
import (
	...
	"github.com/gc-lover/necpgame-monorepo/services/world-events-core-service-go/server"
...
func main() {
	...
	go func() {
		pprofAddr := getEnv("PPROF_ADDR", "localhost:6655")
	...
	monitor := server.NewGoroutineMonitor(200, logrusLogger) // Max 200 goroutines for world-events-core service
	go monitor.Start()
	defer monitor.Stop()
	logrusLogger.Info("✅ Goroutine monitor started")

	// Start server
	go func() {
		logger.Info("Starting World Events Core Service", zap.String("port", serverPort))
