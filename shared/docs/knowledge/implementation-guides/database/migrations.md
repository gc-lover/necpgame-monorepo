...
|--------|------|------------|--------|
| V1_0 | `V1_0__init_core.yaml` | Создание схем `mvp_core`, `mvp_meta`, базовые таблицы (`player_account`, `character`, `order`, `order_phase`, `order_application`, `order_review`, `weapon_profile`, `ballistics_metric`, `crafting_blueprint`, `crafting_job`, `world_district_state`, `event_log`, `outbox`) | OK ready |
| V1_1 | `V1_1__indexes_constraints.sql` | Индексы, уникальные ограничения, внешние ключи | OK ready |
| V1_2 | `V1_2__enum_types.sql` | Создание enum типов (`order_state`, `order_access`, `crafting_status`, `weapon_type`) | OK ready |
| V1_3 | `V1_3__audit_triggers.sql` | Триггеры `updated_at`, `soft_delete` | OK ready |
| V1_4 | `V1_4__seed_reference_data.sql` | Базовые значения (оригины, классы, фракции) | OK ready |
| V1_5 | `V1_5__ballistics_metrics_extension.sql` | Расширение метрик (jsonb колонки, partial index) | OK ready |
