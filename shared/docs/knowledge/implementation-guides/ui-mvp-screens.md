...
|-------|----------|--------|-----------|-------|
| Регистрация | Форма входа/регистрации, OAuth | `POST /auth/register`, `POST /auth/login` | auth-service | OK |
| Выбор сервера | Список серверов, пинг, населения | `GET /infrastructure/servers` | infrastructure-service | OK |
| Создание персонажа | Характеристики, происхождение, класс | `GET /characters/templates`, `POST /characters` | character-service | OK |
| Туториал (Intro) | Пошаговое обучение, первые квесты | `GET /quests/tutorial`, `POST /quests/progress` | quest-service | OK |
| Основной HUD | Карта, журнал, навыки, чат | `GET /world/state`, WebSocket `world.unrest.updates` | world-service, communication-service | OK |
| Combat Overlay | Боевой UI, статы оружия | `POST /combat/ballistics/simulate`, Kafka `combat.ballistics.events` | gameplay-service | OK |
| Orders Board | Социальные заказы | `GET /social/orders`, `POST /social/orders/{id}/applications`, Kafka `social.orders.lifecycle` | social-service | OK |
| Crafting Workshop | Крафт модов, очередь | `GET /crafting/blueprints`, `POST /crafting/jobs`, Kafka `economy.crafting.jobs` | economy-service | OK |
| Dialogue UI | Ветки диалогов, выбор решений | `GET /dialogues/{id}`, `POST /dialogues/choice` | narrative-service | OK |
| Inventory | Экипировка, моды | `GET /inventory`, `POST /inventory/equip` | inventory-service | OK |
